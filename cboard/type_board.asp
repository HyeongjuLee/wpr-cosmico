<table <%=tableatt%> class="board notice width100">
	<colgroup>
		<col width="60" />
		<col width="*" />
		<col width="120" />
		...
		<tr>
			...
			<td colspan="3" height="40" class="tright">
				<%
					Select Case DK_MEMBER_TYPE
						Case "MEMBER","GUEST","COMPANY"
							If DK_MEMBER_LEVEL >= intLevelWrite Then
								...
								If WriteTF = "T" Then
									PRINT TABS(3)& "	<a href=""board_write.asp?bname="&strBoardName&""" class=""button write"">"&LNG_BOARD_BTN_WRITE&"</a>"
								Else
									PRINT TABS(3)& "	<a href=""javascript:alert('해당 게시판의 일일 글쓰기 횟수를 초과했습니다.\n익일 글쓰기를 시도해주세요.');"" class=""button write"">"&LNG_BOARD_BTN_WRITE&"</a>"
								End If
							End If
					...
					PRINT TABS(3)& "	<a href=""board_list.asp?bname="&strBoardName&""" class=""button"">"&LNG_BOARD_BTN_LIST&"</a>"
				%>
			</td>
		</tr>
	</tfoot>
...
%>
				<tr class="table_for">
					...
					<td><%=viewName%></td>
					<td><%=arrList3_regDate%></td>
					<td><%=num2cur(arrList3_readCnt)%></td>
				</tr>
<%
