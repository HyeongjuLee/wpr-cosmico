	...
	*/
	 ...
	 */
     function goStartMode(){
		welBtnConfirm.removeClass("design2").addClass("design1");
		//welBtnConfirm.css({"color":"blue"});
		fnUploadImage.attach(welBtnConfirm.$value(), "click");
		/*
    	 var sSrc = welBtnConfirm.attr("src")|| "";
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","../../img/ko_KR/btn_confirm2.png");
    		 fnUploadImage.attach(welBtnConfirm.$value(), "click");
    	 }
		 */
     }
     ...
     function goReadyMode(){
		welBtnConfirm.removeClass("design1").addClass("design2");
		//welBtnConfirm.addClass("design2");
		fnUploadImage.detach(welBtnConfirm.$value(), "click");
		 /*
    	 var sSrc = welBtnConfirm.attr("src")|| "";
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    		 fnUploadImage.detach(welBtnConfirm.$value(), "click");
	    	 welBtnConfirm.attr("src","../../img/ko_KR/btn_confirm.png");
    	 }
		*/
     }
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="../../img/ko_KR/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');
    ...
    function html5Upload() {
    	...
    	sUploadURL= 'upload_image.asp'; 	//upload URL
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("이미지 파일(jpg,gif,png)만 업로드 하실 수 있습니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/upload_image.asp',	//샘플 URL입니다.
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	error : function(oCustomEvent) {
 	    		...
 	    		alert(decodeURIComponent(oCustomEvent.htResult.errstr));
 	    	}
 	    });
 	}
