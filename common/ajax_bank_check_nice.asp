...
'Response.end
	...
	If DbCheckMember > 0 Then
		Response.Write "<span class=""red"">이미 등록된 계좌정보가 있습니다.. 본인이 회원가입을 하지 않았다면 본사로 문의해주세요.</span>"
		Response.End
	End If
	...
	Public Sub GetMLBMCode()
		...
		If webproIP = "T" And DevMode = "T" Then
		...
		If ResultCD = "0000" Then
			...
			If OUTPUT_VALUE = "FINISH" Then
				...
				If Int(CSDBSameBankCnt) = 1 Then
					PRINT "<span class=""red tweight"">이미 CS에 등록된 회원이 존재합니다. 다시한번 확인해주세요.</span>"
					PRINT "<input type=""hidden"" name=""ajaxTF"" value=""F"" readonly=""readonly"" />"

				Else
					PRINT "<span class=""blue2 tweight"">본인인증에 성공하였습니다. 다음단계로 진행해주세요.</span>"
			...
			Else
				...
				PRINT "<span class=""red2 tweight"">데이터베이스 입력에 실패하였습니다. "&Msg&"</span>"
		...
		Else
				PRINT "<span class=""red2 tweight"">본인인증에 실패하였습니다. "&Msg&"</span>"
