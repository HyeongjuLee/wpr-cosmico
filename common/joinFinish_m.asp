...
''	R_NationCode = Trim(pRequestTF("cnd",True))
	...
	Select Case UCase(LANG)
		Case "KR"
			If Not checkRef(houUrl &"/common/joinStep_n03_m.asp") Then Call alerts(LNG_ALERT_WRONG_ACCESS,"go","/common/joinStep01.asp")
		Case Else
...
<%
	...
	'#####################################
		...
		Else
			Call ALERTS("인증데이터가 존재하지 않습니다. 인증을 다시 시도해주세요.","GO","/common/joinStep01.asp")
		End If
		Call closeRs(DKRSM)
...
'Response.end
		...
		'KR제외 외국 strSSH = cpno_Global저장!!
			...
			'Db.makeParam("@signature_file",adVarWChar,adParamInput,100,signature_file), _
	   '▣▣ [다국어] 사업자회원가입 INPUT - CS DB ONLY!!▣▣
		...
		If OUTPUT_VALUE = "FINISH" Then
			...
			If S_SellMemTF = "0" Or S_SellMemTF = "1" Then		'판매원/소비자
...
'	RESPONSE.End
		Select Case OUTPUT_VALUE
			...
			Case "FINISH"		: Call ALERTS(LNG_JOINFINISH_U_ALERT_OUTPUT07&"("&THISMEMID1&" - "&Fn_MBID2(THISMEMID2)&")","go","/index.asp")
