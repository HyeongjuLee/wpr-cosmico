...
<%
	...
	If Not AHJRS.BOF And Not AHJRS.EOF Then
		...
		'출금 ============================================================================
			HJRSC_BTC_qoute		= 0
			MINIMUM_POINT		= CONST_CS_MINIMUM_WITHDRAW_POINT
		...
		If CDbl(num2Cur(CONST_CS_WITHDRAW_UNIT)) <> CDbl(AHJRS_WITHDRAW_UNIT) Then
		...
		'▣출금액 단위 확인
		If transPoint Mod CONST_CS_WITHDRAW_UNIT <> 0 Then
			PRINT "{""result"":""error"", ""message"":"""&LNG_CS_POINT_TRANSFER2CASH_OK_ALERT06& "("&num2cur(CONST_CS_WITHDRAW_UNIT)&")"", ""resultData"" : """"}"
			Response.End
		End If
