...
<%
'ASP Security Image Generator v4.5 - 19/September/2011
'Generate images to make a CAPTCHA test
'? 2006-2011 Emir T?z?l. All rights reserved.
...
'#Begin ColorMap
'const BmpColorMap = "dffeff000c851700"
'					 "bgclor__foclor__"
...
'const BmpColorMap = "11111100e4e4e400"
const BmpColorMap = "99999900e4e4e400"
'const BmpColorMap = "cdcdcd00af511100"
...
'dim Bitmap(27,110)			'[Height,Width]		'25,130
dim Bitmap(49,200)			'[Height,Width]		'25,130
CodeLength = 6				'Secure code length (Max:8)
...
const NoiseEffect = 1		'0[none], 1[sketch], 2[random lines of text color], 3[Random lines of background color over text (Recommed maximum NoiseLine=4)]
...
const MinLineLength = 60	'Minimum noise line length
...
function RandomWord()
    RandomWord = words(RndInterval(0,UBound(words)))
end function
...
sub PrepareBitmap(valSecureCode)
	...
	TextHeight = TextHeight * 1.3									'¡Ú

	'left margin
	LeftMargin = Round((ImageWidth - arrTextWidth(0)) / 2)
	LeftMargin = LeftMargin/1.3										'¡Ú
...
sub DrawLine(x0, y0, x1, y1, valClr)
	...
	if Abs(dx) > Abs(dy) then
		...
		do while x0 <> x1
			...
			if (NoiseEffect = 3) and (Bitmap(Round((m * x0) + b),x0) = TColor) then
				clrNoise = vbNullString
			else
				clrNoise = valClr
			end if
	...
	elseif dy <> 0 then
		...
		do while y0 <> y1
			...
			if (NoiseEffect = 3) and (Bitmap(y0,Round((m * y0) + b)) = TColor) then
				clrNoise = vbNullString
			else
				clrNoise = valClr
			end if
...
if CodeType < 2 then
    ...
    %>
    <!--#include file="wordlist.asp"-->
    <%
	secureCode = RandomWord()
    CodeLength = Len(secureCode)
end if
