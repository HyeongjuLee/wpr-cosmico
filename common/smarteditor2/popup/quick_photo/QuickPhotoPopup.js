	...
	var rFilter2 = /^(gif|jpg|jpeg|png)$/i;
	var nTotalSize = 0;
    ...
    function html5Upload() {
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/common/smartEditor2/upload_image.asp',	//샘플 URL입니다.
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		//}
				  var aResult = []; //추가
				   aResult[0] = oCustomEvent.htResult;//추가
...
//===============================================================================================================================================================
	function callFileUploader2 (){
		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
			sUrl  : 'http://xn--o39aqq199a.com/common/smartEditor2/upload_image.asp',	//샘플 URL입니다.
 			sCallback : 'http://xn--o39aqq199a.com/common/smarteditor2/popup/quick_photo/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.png;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
 	    	bAutoUpload : false,									 	//파일이 선택됨과 동시에 자동으로 업로드를 수행할지 여부 (upload 메소드 수행)
 	    	bAutoReset : true 											// 업로드한 직후에 파일폼을 리셋 시킬지 여부 (reset 메소드 수행)
	...
	function callFileUploader3 (){
		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
			sUrl  : 'http://www.xn--o39aqq199a.com/common/smartEditor2/upload_image.asp',	//샘플 URL입니다.
 			sCallback : 'http://www.xn--o39aqq199a.com/common/smarteditor2/popup/quick_photo/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.png;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
 	    	bAutoUpload : false,									 	//파일이 선택됨과 동시에 자동으로 업로드를 수행할지 여부 (upload 메소드 수행)
 	    	bAutoReset : true 											// 업로드한 직후에 파일폼을 리셋 시킬지 여부 (reset 메소드 수행)
...
//===============================================================================================================================================================
	...
	var this_url = document.location.href; //현재창 주소호출!!!
	...
	window.onload = function(){
  		...
  		if(bSupportDragAndDropAPI){
			...
			//한글도메인 분기2,한글도메인 분기3 www
			if (this_url == "http://가보고.com/common/smarteditor2/popup/quick_photo/Photo_Quick_UploadPopup.html"){
  				callFileUploader2();
			}else if (this_url == "http://www.가보고.com/common/smarteditor2/popup/quick_photo/Photo_Quick_UploadPopup.html"){
  				callFileUploader3();
			}else{
  				callFileUploader();
			}
