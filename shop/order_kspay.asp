...
Select Case UCase(DK_MEMBER_NATIONCODE)
	Case "KR"
		...
		<%
			Case "KSNET"
				BODYLOAD = ""
				PRINT "<script type=""text/javascript"" src=""/PG/KSNET/pay.js?v1.1""></script>"
		...
		<script type="text/javascript" src="order_mComplex.js?v0"></script>
...
<div id="cart">
				...
				If IsArray(arrList) Then
					For i = 0 To listLen
						...
						'################################################################## START
							...
							self_TOTAL_PRICE = 0 'self_TOTAL_PRICE = self_GoodsPrice + self_GoodsOptionPrice
							self_PV = 0
							self_GV = 0
						...
						' 상품별 금액/적립금 확인
							...
							self_TOTAL_PRICE = selfPrice + self_GoodsOptionPrice
	...
	'							''		Response.Write "(기본배송비정책이 입력되지 않았습니다)"
						...
						'TOTAL_Price = TOTAL_Price + selfPrice + self_DeliveryFee + self_GoodsOptionPrice
						TOTAL_Price = TOTAL_Price + selfPrice + self_GoodsOptionPrice
						TOTAL_USEPOINT_PRICE = TOTAL_USEPOINT_PRICE + selfPrice + self_GoodsOptionPrice
						'TOTAL_FIRST_PRICE = TOTAL_FIRST_PRICE + selfPrice + self_GoodsOptionPrice + self_DeliveryFee
			...
			%>
				...
				<tr class="<%=trClass%><%=trClass2%>">
					...
					<td class="tcenter <%=tdClass%>">
						<%=spans(num2cur(selfPrice/arrList_orderEa),"#FF6600","","bold")%>&nbsp;<%=Chg_currencyISO%>
					...
					<!-- <td class="tcenter" style="line-height:160%;">
						<strong><%=spans(num2cur(selfPrice)&" 원","#FF6600","","")%></strong>
					...
					<td class="tcenter <%=tdClass%>">
						<%=spans(num2cur(selfPrice),"#ff6600","","bold")%>&nbsp;<%=Chg_currencyISO%>
						<%If DK_MEMBER_STYPE = "0" And arrList_isCSGoods = "T" Then%>
	...
	<%
If webproIP="T" Then TOTAL_DeliveryFee = 801
				...
				<table <%=tableatt%> class="width100">
					...
					<tbody>
						...
						<tr>
							...
							<%If DK_MEMBER_TYPE <> "GUEST" Then%>
								<input type="hidden" name="useCmoney" class="input_gray01"  onKeyUp="toCurrency(this)" onBlur="toCurrency(this); checkUseCmoney(this);" value="0"   /><span style="margin-left:5px;">사용가능 <%=SHOP_POINT%> : <strong><%=num2CurINT(MILEAGE_TOTAL)%></strong> <%=SHOP_POINT%></span> <!-- | <%=LNG_SHOP_ORDER_FINISH_30%> --><!-- </td> -->
							...
							<%Else%>
								<input type="hidden" name="useCmoney" class="input_gray01" value="0" disabled="disabled" style="background-color:#eee;" /><span style="margin-left:5px;">(<%=SHOP_POINT%> <strong>-</strong> <%=Chg_CurrencyISO%></span>) | <span style="color:#ee0000"><%=LNG_SHOP_ORDER_FINISH_31%></span><!-- </td> -->
								<br />
								<input type="hidden" name="useCmoney2" class="input_gray01" value="0" disabled="disabled" style="background-color:#eee;" /><span style="margin-left:5px;">(<%=SHOP_POINT2%> <strong>-</strong> <%=Chg_CurrencyISO%></span>) | <span style="color:#ee0000"><%=LNG_SHOP_ORDER_FINISH_31%></span>
								</td>
							<%End If%>
						</tr>
						<%Else%>
	...
	<div class="cleft width100 payment">
		...
		<div class="width100 selectPay">
			...
			%>
				...
				<%If 1=2 Then%>
				<label><input type="radio" name="paykind" value="CardAPI" onclick="chgPay(this.value)" class="input_radio" /> <%=LNG_SHOP_ORDER_DIRECT_PAY_01%>KSNET KeyIn</label>
				<label><input type="radio" name="paykind" value="mComplex" onclick="chgPay(this.value)" class="input_radio" />KSNET 복합결제</label>
				<%End If%>
			...
			<%'▣포인트 단독결제%>
				<!-- <%If DK_MEMBER_TYPE = "COMPANY" And isSHOP_POINTUSE = "T" Then%>
					<%If CDbl(MILEAGE_TOTAL) >= CDbl(TOTAL_SUM_PRICE) Then%>
						<label><input type="radio" name="paykind" value="point" onclick="chgPay(this.value)" class="input_radio"  /> <%=LNG_TEXT_PAYING_POINT2_ALONE%></label>
					   &nbsp;&nbsp;<span id="pointTXT" class="tweight red2 font14px" ></span>
					<%Else%>
						<label><input type="radio" name="paykind" value="point" onclick="chgPay(this.value)" class="input_radio" disabled="disabled" /> <%=LNG_TEXT_PAYING_POINT2_ALONE%> (포인트 부족)</label>
					   &nbsp;&nbsp;<span id="pointTXT" class="tweight red2 font14px" ></span>
					<%End If %>
				<%End If %> -->
			<%
									...
									<table <%=tableatt%> class="width100">
										...
										<tr>
											...
											<td>
												...
												<input type="text" title="금액" name="CardPrice" class="input_text" style="width:100px" <%=onlykeys%> value="" />
												<a class="a_submit design3 submitPrice_Card">금액적용</a>
											</td>
											<th>카드번호</th>
											<td>
												<input type="text" title="카드번호 1" name="mCardNo1" class="input_text" style="width:55px" maxlength="4" value="" <%=onlyKeys%> /> -
												<input type="password" title="카드번호 2" name="mCardNo2" class="input_text" style="width:55px" maxlength="4" value="" <%=onlyKeys%> /> -
												<input type="password" title="카드번호 3" name="mCardNo3" class="input_text" style="width:55px" maxlength="4" value="" <%=onlyKeys%> /> -
							...
							<%Case "SPEEDPAY","YESPAY","DAOU","NICEPAY","ONOFFKOREA","KSNET"%>
								<tr>
									...
									<td>
										...
										<input type="password" name="cardNo2" class="input_text tcenter" maxlength="4" style="width:60px;" placeholder="4자리" <%=onlyKeys%> value="" /> -
								...
								<%End If%>
								<%If DKPG_PGCOMPANY = "DAOU" Or DKPG_PGCOMPANY = "KSNET" Then%>
								<tr>
