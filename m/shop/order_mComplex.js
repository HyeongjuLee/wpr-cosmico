...
$(document).ready(function() {
	...
	$("#CardAdd").on("click", function(){
		...
		if (thisLeng >= 3)
		{
			...
			divs.clone().insertAfter(divs).find("input[type=text], input[type=password], input[type=hidden], select, input[type=number], input[type=tel]").val('');		//카드입력(복제)추가, value = ""
...
function resetAllmComplexInfo() {
	...
	resetInput.find("input[type=text], input[type=password], select , input[type=number]").val('');
...
function resetAllBankInfo() {
	...
	resetInput.find("input[type=text], input[type=password], select check, input[type=number]").val('');
	resetInput.find("input[name=BankVld]").val('F');
	resetInput.find("input[name=mBankTotal]").val(0);
