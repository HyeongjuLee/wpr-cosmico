...
'	2. [후원인 등록관련]
'
'		A.	본인이 추천인만으로 등록하여 매출을 등록 후 상위 추천인이 후원인 조직도에서 후원인을 등록하여 주는 방식 처리
'
'
'		B.	후원인으로 등록이 되는 경우 후원인 등록되는 일자를 기준으로 매출내역 변경 처리
'			i.	판매일자의 경우 UTC+6기준으로 처리
'			ii.	매출일 변경시 해당 매출에 대한 판매승인 처리
'
'
'		C.	본인의 추천하선 회원에 후원등록 대한 내역
'			i. 대상 회원
'				1.	후원인이 없는 회원(최상위 회원)
'				2.	본인 매출에 대한 입금확인이 확인된 회원
'
'			ii.해당 회원에 대한 목록 표시처리
'				1.	목록 내역에 미입금 및 입금된 회원을 구분하여 표시
'
'			iii.후원인 등록시 해당 회원의 결재가 완료된 내역에 대하여서는 모두 판매 승인 처리
'
'
'		D.	하선 후원인 등록시 본인의 후원인이 정상등록이 되어 있어야 가능하도록 처리
	...
	'후원인 등록 가능한 직추천 회원목록
		'T_tree_ss_V10_UD_LINE_SELECT_LIST_AJAX.asp

	'회원 기타정보 박스에서 후원인 선택
		'T_tree_ss_V10_UD_PUR1_AJAX.asp
...
'	PRINT SDK_MEMBER_ID2
	...
	cZoom			= Request("cZoom")
	'★후원인 등록
	cUnderSelect	= Request("cUnderSelect")
	...
	'★후원인 등록
	If cUnderSelect = "" Then cUnderSelect = 0
	'본인 / 직후원 총하선PV
'	arrParams = Array(_
...
<div class="SearchLayer2">
	<div id="search_wrap">
		<div class="cleft">
			<a href="T_tree_ss_V12.asp" class="a_submit design4"><%=LNG_TEXT_INITIALIZATION%></a>
		...
		<div id="search_period" class="cleft" style="<%=SEARCHCSS%>">
			...
			<%
				...
				VOTER_MEMBER_SPON_REG_CNT = Db.execRsData("HJP_VOTER_MEMBER_SPON_REG_CNT",DB_PROC,arrParamsVC,DB3)
			%>
			<%If MY_SAVE_MEMBER_TF = "T" And VOTER_MEMBER_SPON_REG_CNT > 0 Then%>
				<input type="checkbox" name="cUnderSelect" id="cUnderSelect" value="1" <%=isChecked(cUnderSelect,"1")%>><label for="cUnderSelect" style="margin-left:7px;"><span class="red2"><%=LNG_TEXT_UNDER_MEMBER_SPONSOR_SELECT%></span></label>
			<%End If%>

			<form name="sfrm" action="T_tree_ss_v12.asp" method="post">
				...
				<%'★후원인 등록%>
				<input type="hidden" name="cUnderSelect" id="cUnderSelect" value="<%=cUnderSelect%>" >
...
'	DK_MEMBER_ID2 = 10001
	...
	If IsArray(arrList) Then
		...
		For i = 0 To listLen
			...
			'★후원인 등록
			If cUnderSelect	 = "1" Then cUnderSelect_display = "" Else cUnderSelect_display = "display:none;" End If
		...
		'★후원인 등록 - 직하선 라인CNT체크 (없으면 0) ==============================================================
			arrParams = Array(_
				Db.makeParam("@DK_MEMBER_ID",adVarChar,adParamInput,20,arrList_Mbid),_
				Db.makeParam("@DK_MEMBER_ID2",adInteger,adParamInput,0,arrList_Mbid2),_
				Db.makeParam("@LINECNT",adInteger,adParamInput,0,1)_
			)
			LINECOUNT_LEFT = Int(Db.execRsData("HJP_LINECNT_CHECK",DB_PROC,arrParams,DB3))

			arrParams = Array(_
				Db.makeParam("@DK_MEMBER_ID",adVarChar,adParamInput,20,arrList_Mbid),_
				Db.makeParam("@DK_MEMBER_ID2",adInteger,adParamInput,0,arrList_Mbid2),_
				Db.makeParam("@LINECNT",adInteger,adParamInput,0,2)_
			)
			LINECOUNT_RIGHT = Int(Db.execRsData("HJP_LINECNT_CHECK",DB_PROC,arrParams,DB3))

			LINE_BTN_LEFT = ""
			LINE_BTN_RIGHT = ""

			If LINECOUNT_LEFT = 0 Then
				LINE_BTN_LEFT = "" & _
					"<input type=""hidden"" name=""left_spon_mbid1"" value="""&arrList_Mbid&""" />" & _
					"<input type=""hidden"" name=""left_spon_mbid2"" value="""&arrList_Mbid2&""" />"
				LINE_BTN_LEFT = LINE_BTN_LEFT & "<span class=""buttonPRGap""><input type=""button"" name=""LINE_BTN_LEFT"" class=""input_submit design4"" value=""1"" /></span>"
			Else
				LINE_BTN_LEFT = "<span class=""buttonPRGap""><input type=""button"" name=""LINE_BTN_LEFT_FULL"" class=""input_submit designX"" style=""cursor: default"" value=""X"" onclick=""notsSlectable(1);"" /></span>"
			End If

			If LINECOUNT_RIGHT = 0 Then
				LINE_BTN_RIGHT = "" & _
					"<input type=""hidden"" name=""right_spon_mbid1"" value="""&arrList_Mbid&""" />" & _
					"<input type=""hidden"" name=""right_spon_mbid2"" value="""&arrList_Mbid2&""" />"
				LINE_BTN_RIGHT = LINE_BTN_RIGHT & "<span><input type=""button"" name=""LINE_BTN_RIGHT"" class=""input_submit design4"" value=""2"" /></span>"
			Else
				LINE_BTN_RIGHT = "<span><input type=""button"" name=""LINE_BTN_RIGHT_FULL"" class=""input_submit designX"" style=""cursor: default"" value=""X"" onclick=""notsSlectable(2);"" /></span>"
			End If

			''LINE_BTN_AREA = "<div style="""">"&LINE_BTN_LEFT&LINE_BTN_RIGHT&"</div>"		'div 들어가면 안되네...
			LINE_BTN_AREA = LINE_BTN_LEFT&LINE_BTN_RIGHT

			viewP99	= "<p class="""&liClass&" view99"" style="""&cUnderSelect_display&""">"&LINE_BTN_AREA&"</p>"
		'★후원인 등록 - 직하선 라인CNT체크 (없으면 0) ==============================================================
			...
			'후원인 WebID
			arrParams = Array(_
				Db.makeParam("@mbid",adVarChar,adParamInput,20,arrList_Nom), _
				Db.makeParam("@mbid2",adInteger,adParamInput,0,arrList_Nom2) _
			)
			Set DKRS = Db.execRs("HJP_MEMBER_WEBID",DB_PROC,arrParams,DB3)
			If Not DKRS.BOF And Not DKRS.EOF Then
				arrList_Nom_WebID	= DKRS("WebID")
			Else
				arrList_Nom_WebID =""
			End If
			Call closeRS(DKRS)
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table <%=tableatt%> class="width100" id="TableGrid2">
						...
						<tbody>
							<tr>
								<td colspan="4" class="notData"><%=LNG_TEXT_INPUT_SEARCH_WORD%></td>
							</tr>
							<!-- <tr>
								<td colspan="4" class="notData"><p><img src="159.gif" alt="" /></p><p style="margin-top:30px;"><%=LNG_TEXT_DATA_LOADING%></p></td>
							</tr> -->
			...
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<div style="text-align:center;">
							<table <%=tableatt%> class="width100">
								...
								<tr>
									...
									<td class="view_center"></td> -->
									<th><%=LNG_TEXT_POSITION%></th>
									<td class="view_Grade_Name"></td>

									<th><%=LNG_TEXT_TREE_MY_PV%></th>
									<td class="view_meap"></td>
								</tr>
								<!-- <tr>
									<th><%=LNG_TEXT_TREE_NOMIN_NAME%></th>
									<td class="view_vname"></td>

									<th><%=LNG_TEXT_TREE_NOMIN_MEMID%></th>
					...
					<div class="underPV1">
						...
						<div class="width100" id="PURCHASE1" style="height:196px; border:1px solid #cdcdcd; padding:20px 0px;margin-top:7px;">


						</div>
					</div>
					<!-- <div class="underPV2">
						<h6 style="border-bottom:1px dashed #dee2e6; padding-bottom:7px;">하선 매출 내역 (이월)</h6>
						<div style="text-align:center;">
...
<%'★후원인 등록 - 하선 후원인 등록 리스트창%>
<!-- Modal 4 S -->
	<div class="modal fade" id="ModalScrollable4" tabindex="-1" role="dialog" aria-labelledby="ModalScrollableTitle4" aria-hidden="true" >
		<div class="modal-dialog modal-dialog-scrollable" role="document" >
			<div class="modal-content">
				<input type="hidden" name="hidden_spon_mbid1"	id="hidden_spon_mbid1" value="" readonly="readonly" />
				<input type="hidden" name="hidden_spon_mbid2"	id="hidden_spon_mbid2" value="" readonly="readonly" />
				<input type="hidden" name="hidden_LineCnt"	id="hidden_LineCnt" value="" readonly="readonly" />
				<div class="modal-header">
					<h5 class="modal-title tweight" id="ModalScrollableTitle4"><%=LNG_TEXT_UNDER_MEMBER_SPONSOR_SELECT%> (<span id="hidden_LineCnt_TEXT"></span> LINE)</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				</div>
				<div class="modal-sbody">
					<%=LNG_TEXT_NAME%> (<%=LNG_TEXT_ID%>) : <input type="text" class="input_text" name="sNameSearch_4" id="sNameSearch_4" />
					<input type="button" class="input_submit design1" id="sNameSearchBtn_4" value="<%=LNG_TEXT_SEARCH%>"  />
				</div>
				<div class="modal-body">
					<table <%=tableatt%> class="width100" id="TableGrid4">
						<!-- <col width="" /> -->
						<col width="" />
						<col width="" />
						<col width="" />
