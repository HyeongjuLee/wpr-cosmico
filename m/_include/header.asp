...
<div data-role="page" id="page">
	...
	<header>
		...
		<%Else If PAGE_SETTING = "JOIN" Then%>
		<!--#include virtual = "/m/_include/header_join.asp"-->
		<%Else If PAGE_SETTING = "MYOFFICE" Then%>
	...
	<%Case "COMMON", "JOIN"%>
		<script type="text/javascript">
			...
			$(document).on('keyup change', '.input-wrap input', function(){
				$(this).val() ? $(this).parent('label').addClass('key') : $(this).parent('label').removeClass('key');
			});
		</script>
	<%Case Else%>
	<%End Select%>
	...
	%>
		...
		<link rel="stylesheet" type="text/css" href="/m/css/nav-left.css?v0">
		<div class="content">
