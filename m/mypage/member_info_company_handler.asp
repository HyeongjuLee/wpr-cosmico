...
'		If DK_MEMBER_ID = strPass			Then Call ALERTS(LNG_ALERT_WRONG_ACCESS","back","")
...
'		If Not checkPass(strPass, 6, 20)	Then Call ALERTS(LNG_JS_PASSWORD_FORM_CHECK","back","")
		If isChgPass = "T" Then
			If Not checkPass(newPass, 6, 20)	Then Call ALERTS(LNG_MYPAGE_INFO_COMPANY_HANDLER_ALERT04,"back","")
		End If
...
<%
	...
	strZip = Replace(strZip,"-","")
		...
		arrParams = Array(_
			...
			Db.makeParam("@strBirth1",adVarChar,adParamInput,8,strBirth1),_
			Db.makeParam("@strBirth2",adVarChar,adParamInput,8,strBirth2),_
			Db.makeParam("@strBirth3",adVarChar,adParamInput,8,strBirth3),_
	...
	'	Call Db.exec("DKP_MEMBER_MODIFY2",DB_PROC,arrParams,Nothing)
		...
		Select Case OUTPUT_VALUE
			Case "ERROR"
				Call ALERTS(LNG_MYPAGE_INFO_COMPANY_HANDLER_ALERT08,"go","/m/member/member_info.asp")

			Case "FINISH"

				'▣ CS 계좌인증 UPDATE
				If ajaxTF = "T" And bankNumber <> "" Then
					On Error Resume Next
				...
				Call ALERTS(LNG_MYPAGE_INFO_COMPANY_HANDLER_ALERT09,"go","/m/mypage/member_info.asp"&ptshop)
