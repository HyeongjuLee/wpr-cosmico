...
<%
	...
	If Not checkRef(houUrl &"/m/common/joinStep03_c.asp") Then Call alerts("잘못된 접근입니다.","go","/m/common/joinStep01.asp")
		...
		If agreement <> "T" Then Call ALERTS("이용약관에 동의하셔야합니다.","go","/m/common/joinStep01.asp")
		If gather <> "T" Then Call ALERTS("개인정보 수집 및 이용에 동의하셔야합니다.","go","/m/common/joinStep01.asp")
		If company <> "T" Then Call ALERTS("사업자회원 가입약관에 동의하셔야합니다.","go","/m/common/joinStep01.asp")
...
'		strName			= pRequestTF("strName",True)
	...
	'	Bus_FLAG	 = pRequestTF("Bus_FLAG",True)
	...
	'	End If
		...
		Else
			Call ALERTS("CS회원정보 로드에 실패하였습니다.다시 시도해주세요111.","go","/m/common/joinStep01.asp")
		End If
		Call closeRS(DKRS)
	...
	' 값 처리
		...
		If strEmail = "" Then Call ALERTS("이메일 주소를 입력하셔야합니다.","back","")
...
'		Call ResRW(strPass		,"strPass")
	...
	' ▣ CS 외국 개인식별번호 중복 체크 [cpno_Global]
		If UCase(Lang) <> "KR" Then
			...
			If strSSH <> "" Then
				SQL = " SELECT COUNT(*) FROM [tbl_MemberInfo] WHERE [cpno_Global] = ?  "
				arrParams = Array(_
	...
	'	)
	'	SSHCOUNT = CInt(Db.execRsData(SQL,DB_TEXT,arrParams,Db3))
	'	If SSHCOUNT > 0 Then Call alerts("이미 등록된 회원입니다. 본인이 회원가입을 하지 않았다면 본사로 문의해주세요.","go","/common/joinStep01.asp")
	...
	'▣ CS 이름 + 생년월일 중복체크(탈퇴회원 중복대상제외'→ 전체회원중복체크
		...
		If DbCheckMember > 0 Then Call alerts("동일한 이름과 생년월일로 등록된 회원이 존재합니다.본인이 회원가입을 하지 않았다면 본사로 문의해주세요.!","go","joinStep01.asp")
...
'	RESPONSE.End
		Select Case OUTPUT_VALUE
			...
			Case "FINISH"		: Call ALERTS("회원가입이 완료 되었습니다.\n\n가입회원의 회원번호는 "&THISMEMID1&" - "&Fn_MBID2(THISMEMID2)&" 입니다.","go","/m/index.asp")
			'Case "FINISH"		: Call ALERTS("회원가입이 완료 되었습니다.\n\n가입회원의 아이디는 "&strUserID_OUT&" 입니다.","go","/m/index.asp")
		'	Case "FINISH"		: Call ALERTS("회원가입이 완료 되었습니다.\n\n가입회원의 웹아이디는 "&THISMEMID1&THISMEMID2&" 입니다.","go",""/m/index.asp")
