...
<%
	...
	If Not checkRef(houUrl &"/m/common/joinStep04.asp") Then Call ALERTS("잘못된 접근입니다.","go","/m/common/joinStep01.asp")
...
'####################################################################
	...
	If Agree01 <> "T" Then Call ALERTS("잘못된 접근입니다.","go","/m/common/joinStep01.asp")
	If Agree02 <> "T" Then Call ALERTS("잘못된 접근입니다.","go","/m/common/joinStep01.asp")
	If Agree03 <> "T" Then Call ALERTS("잘못된 접근입니다.","go","/m/common/joinStep01.asp")
	...
	Else
		Call ALERTS("데이터베이스에 없는 데이터입니다1. 다시 시도해주세요.","BACK","")
	End If
	Call closeRs(DKRS)
	...
	If CDbl(DKRS_intIDX)	<>	CDbl(intIDX)		Then Call ALERTS("데이터에 변조가 있었습니다.다시 진행해주세요.12","go","joinStep01.asp")
...
'####################################################################
	...
	'########## ▣ 아이디 중복 체크 ▣
	'	SQL = " SELECT COUNT(*) FROM [tbl_MemberInfo] WITH(NOLOCK) WHERE [WebID] = ?"
	'	arrParams = Array(_
	'		Db.makeParam("@strUserID",adVarChar,adParamInput,20,strUserID) _
	'	)
	'	intVoteCnt = CInt(Db.execRsData(SQL,DB_TEXT,arrParams,DB3))
...
'####################################################################
	...
	'후원인 센터코드호출 -> 센터코드 입력 (2014-12-05)
'	SQL3 = " SELECT [businesscode] FROM [tbl_MemberInfo]  WHERE [mbid] = ? AND [mbid2] = ?"
'	arrParams3 = Array(_
'		Db.makeParam("@SAVEID",adVarChar,adParamInput,20,SponID1),_
'		Db.makeParam("@SAVEID2",adInteger,adParamInput,0,SponID2)_
'	)
'	Set HJRS = Db.execRs(SQL3,DB_TEXT,arrParams3,DB3)
'
'	If Not HJRS.BOF And Not HJRS.EOF Then
'		HJRS_BUSINESSCODE	= HJRS("businesscode")
'	Else
'		Call ALERTS("후원인 센터가 등록되지 않았습니다. 다시 시도해주세요.","BACK","")
'	End If
'	Call closeRs(HJRS)


	Addcode1 = Replace(strZip,"-","")
	nowTime = Now
...
'RESPONSE.End
	...
	Select Case OUTPUT_VALUE
		...
		'Case "FINISH"		: Call ALERTS("회원가입이 완료 되었습니다.\n\n가입회원의 회원번호는 "&THISMEMID1&" - "&Right("000000000"&THISMEMID2,MBID2_LEN)&" 입니다.","go","/m/index.asp")
		Case "FINISH"
			On Error Resume Next
