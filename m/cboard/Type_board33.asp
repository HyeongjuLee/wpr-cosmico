...
'								depthst = cInt(arrList(4,i))
					...
					%>
									<tr class="table_for">
										<td style="padding:8px 0px;font-size:15px; font-weight:bold;">
											<p class="subject">

										<%If arrList3_intDepth > 0 Then%><%For subtabs=1 To arrList3_intDepth%>1111<%next%><img src="images/bbs_answer.gif" width="10" height="10" alt="답변아이콘" />&nbsp;<%End If%>
										<%
										 '■QnA 본인 작성글만 확인
										...
										%>
											<%IF ORI_WRITE_MEMBER_ID <> DK_MEMBER_ID Then%>
												<a class="cp" onclick="alert('<%=QNA_ALERT_TEXT%>');"><%=backword_title(arrList3_strSubject)%>&nbsp;<%=ico_data%><%=ico_secret%></a><%=new_icon%>
											<%Else%>
										...
										<%Else%>
											...
											<%If arrList3_intDepth > 0 Then%><%For subtabs=1 To arrList3_intDepth%>2222<%next%><img src="images/bbs_answer.gif" width="10" height="10" alt="답변아이콘" />&nbsp;<%End If%>
											<%
												If arrList3_isSecret = "T" Then
										...
										<p class="info"><span class="desc1"><%=arrList3_strName%></span> | <span class="desc2"><%=LNG_BOARD_TYPE_BOARD_TEXT04%><!-- 등록일 --> : <%=arrList3_regDate%></span> | <span class="desc3"><%=LNG_BOARD_TYPE_BOARD_TEXT05%><!-- 조회 --> : <%=arrList3_readCnt%></span></p>
