...
<!--#include virtual = "/m/_include/header.asp"-->
	<div id="board" class="board write reply">
		<div class="sub-title"><%=LNG_BOARD_BTN_REPLY%></div>
			...
			<table <%=tableatt%>>
				...
				<tbody>
					<tr class="title">
						...
						<td colspan="2"><input type="text" name="strSubject" value="RE : <%=backword(DefaultReplySubject)%>" data-theme="w"/></td>
					</tr>
					...
					<tr class="password secret">
						...
						<td colspan="2">
							<input type="password" name="strPass" value="" placeholder="<%=LNG_TEXT_PASSWORD%>" data-theme="w"/>
					...
					<tr class="mobile">
						...
						<td colspan="2">
							<input type="text" name="strMobile" value="" placeholder="<%=LNG_TEXT_MOBILE%> ex) 010-1234-5678" data-theme="w" />
					...
					<tr class="contents">
						<td colspan="2">
							...
							<textarea name="content1" cols="" rows="" placeholder="<%=LNG_JS_CONTENTS%>" onclick="thisDeChk1();" data-theme="w" ><%=backword(defaultWord)%></textarea><!--내용을 입력해 주세요.-->
						</td>
					</tr>
					...
					<tr class="file">
						...
						<td>
							<div>
								<!-- <input class="upload-name" placeholder="<%=intData1MB%> MB↓"> -->
								<input class="upload-name" readonly placeholder="<%=intData1MB%> <%=LNG_BOARD_WRITE_TEXT16%>">
								<label>
