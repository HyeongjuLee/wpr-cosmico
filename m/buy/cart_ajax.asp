...
'			Response.WRITE DBERROR
			resultMsg = DBERROR
			PRINT "{""result"" : ""error"", ""resultMsg"" : """&resultMsg&"""}"

			Response.End
		Case "FINISH"
			'Response.WRITE "수량값 변경이 "&DBFINISH
			''Response.WRITE LNG_CS_CART_AJAX_ALERT02 &"("&THIS_NCODE_CNT&")"
			resultMsg = LNG_CS_CART_AJAX_ALERT02
			PRINT "{""result"" : ""success"", ""resultMsg"" : """&resultMsg&""", ""thisGoodCartCnt"" : """&THIS_GOOD_CART_CNT&"""}"
			Response.End
		Case "FINISH2"
			'Response.WRITE "이미 장바구니에 존재하는 상품이라 수량 추가 하였습니다"
			'Response.WRITE LNG_CS_CART_AJAX_ALERT03 &"("&THIS_NCODE_CNT&")"
			resultMsg = LNG_CS_CART_AJAX_ALERT03
			PRINT "{""result"" : ""success"", ""resultMsg"" : """&resultMsg&""", ""thisGoodCartCnt"" : """&THIS_GOOD_CART_CNT&"""}"
			Response.End
		Case "FINISH3"
			'Response.WRITE "장바구니에 없는 물품이라 장바구니에 추가하였습니다."
			'Response.WRITE LNG_CS_CART_AJAX_ALERT04
			resultMsg = LNG_CS_CART_AJAX_ALERT04
			PRINT "{""result"" : ""success"", ""resultMsg"" : """&resultMsg&""", ""thisGoodCartCnt"" : """&THIS_GOOD_CART_CNT&"""}"
			Response.End
		Case Else
			'Response.WRITE "에러가 발생하였습니다. 코드 - "&OUTPUT_VALUE
			'Response.WRITE LNG_CS_CART_AJAX_ALERT05 & OUTPUT_VALUE
			resultMsg = LNG_CS_CART_AJAX_ALERT05
			PRINT "{""result"" : ""error"", ""resultMsg"" : """&resultMsg&"""}"
