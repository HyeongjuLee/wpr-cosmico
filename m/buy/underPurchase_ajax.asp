...
%>
	...
	<p class="sub_title"><%=LNG_TEXT_LIST%> <%=UnderMemberInfo%></p>
	<table <%=tableatt%> class="width100 purchase board">
		<colgroup>

		</colgroup>
		<thead>
			<tr>
				<th rowspan="2"><%=LNG_TEXT_LINE%></th>
		...
		<%
			...
			If IsArray(arrList) Then
				...
				'	Tg_Onclick = Tg_Onclick &"javascript:underPurchase_ajax_view('"&arr_mbid&"','"&arr_mbid2&"','"&SDATE&"','"&EDATE&"','underPurchase_detail"&i&"','1','voterPurchase_detail.asp');"
		...
		%>
			<tr>
				<td class="tcenter" rowspan="2"><%=arr_N_LineCnt%></td>
				<td class="tcenter"><%=arr_mbid%>-<%=Fn_MBID2(arr_mbid2)%></td><%'ID%>
				<td class="tcenter"><%=arr_M_name%> <!-- (<%=FN_SELL_MEM_TF(arr_Sell_Mem_TF)%>) --></td><%'NAME%>
			</tr>
			<tr>
				<td class="tright">
				<%=num2curINT(SUMPrice_P)%><span class="cur1"><%=Chg_CurrencyISO%></span><br>
				<%=num2curINT(SUMPV_P)%><span class="cur2"><%=CS_PV%></span>
			...
			<tr>
				<td colspan="3"><input type="button" class="detail_btn noline" value="<%=LNG_BTN_DETAIL%>" onclick="<%=Tg_Onclick%>" /></td>
			</tr>
			<tr style="display:none;" id="purchase<%=i%>" class="hid_tbody">
				<td colspan="4" class="width100 tcenters" id="underPurchase_detail<%=i%>">
					<div class="loading tcenter" id="loading" style="background-color:#fff;">
						<%=viewImgOPT(IMG&"/159.gif",50,50,"","style=""margin:20px 0px;""")%><br />
	...
	<table class="totals width100">
		<tr class="trData">
			<th colspan="2"><%=LNG_TEXT_TOTAL%></th>
			<th class="tright">
		...
		<%
				Else
					PRINT TABS(1)& "	<tr>"
					PRINT TABS(1)& "		<td colspan=""7"" class=""notData"">"&LNG_NO_UNDER_MEMBER_TEXT&"</td>"
					PRINT TABS(1)& "	</tr>"
				End If
		%>
