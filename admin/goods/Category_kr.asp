			...
			<tbody>
				<tr>
					<td class="tcenter">
						...
						<p>
							<select name="cate2" id="cate2" class="select2">
								<option value="">상위 카테고리를 선택해주세요.</option>
							</select>
						</p>
						<br />
						<input type="submit" value="하위메뉴로 이동" class="submit" />
					</td>
				</tr>
			</tbody>
		</table>
...
<div id="divList">
	...
	%>
				<form name="frmc<%=i%>" method="post" action="Category_Handler.asp" onsubmit="return menuMode(this)" enctype="multipart/form-data">
					...
					<table <%=tableatt%> class="width100">
						...
						<tr>
							...
							<td><%=viewImgStJS(IMG_BTN&"/cate_up.gif",16,16,"","","bor1 vmiddle cp","onclick=""sortUp('"&arrList_strCateCode&"')""")%><%=viewImgStJS(IMG_BTN&"/cate_down.gif",16,16,"","margin-left:2px;","bor1 vmiddle cp","onclick=""sortDown('"&arrList_strCateCode&"')""")%></td>
							...
							<td><input type="image" src="<%=IMG_BTN%>/btn_modify.gif" class="vmiddle" /><img src="<%=IMG_BTN%>/btn_del.gif" width="34" height="17" alt="현재행삭제" style="margin-left:5px;" class="cp vmiddle" onclick="delok('<%=arrList_strCateCode%>')" /></td>
						</tr>
					</table>
				</form>
		<%
