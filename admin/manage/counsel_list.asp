...
<%
	...
	arrList = Db.execRsList("DKPA_COUNSEL_LIST",DB_PROC,arrParams,listLen,Nothing)
...
<div id="manage" class="fleft adminFullWidth">
	<p class="titles">검색</p>
	<form name="sfrm" action="counsel_list.asp?nc=<%=LCase(strNationCode)%>" method="post">
		<table <%=tableatt%> class="adminFullTable search table_fixed">
			...
			<tr>
				...
				<td>
					...
					<%=aImg(Request.ServerVariables("SCRIPT_NAME")&"?nc="&LCase(strNationCode),IMG_BTN&"/search_reset.gif",80,23,"")%>
				</td>
			</tr>
		</table>
	</form>
		...
		<table <%=tableatt%> class="adminFullTable list">
			...
			%>
				<tr>
					...
					<td><%=aImgOpt("javascript:counselView('"&arrList(1,i)&"','"&strNationCode&"');","S",IMG_BTN&"/btn_gray_detailView.gif",70,22,"","")%></td>
				</tr>
			<%
					Next
				Else
