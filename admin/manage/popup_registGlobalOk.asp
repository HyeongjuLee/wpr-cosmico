...
<%
	...
	End Function
		...
		MaxDataSize1 = 5 * 1024 * 1024 ' 실제 썸네일의 업로드 시킬 파일 사이즈
		...
		isViewMobile	= upfORM("isViewMobile", True)

		If popKind = "L" Then
			marginTop		= upfORM("marginTop",True)
			marginLeft		= upfORM("marginLeft",True)
			strScontent		= upfORM("strScontent",False)

			'imgName = FN_IMAGEUPLOAD("imgName","T",MaxDataSize1,REAL_PATH("popup"),"","F","0","0","t1","t2","t3")
			imgName = FN_IMAGEUPLOAD("imgName","T",MaxDataSize1,REAL_PATH("popup"),"","F","0","0","","t1","t2","t3")	'+ originF

			Dim Image, Status
			Set Image = Server.CreateObject("TABSUpload4.Image")
			Status = Image.Load(REAL_PATH("popup")&"\"&imgName)
			If Status = Ok Then
				imgWidth		= Image.Width
				imgHeight		= Image.Height
				Image.Close
			Else
				Call sbDeleteFiles(REAL_PATH("popup")&"\"&imgName)
		...
		If linkType <> "F" Then
			linkUrl		= upfORM("linkUrl",True)
		Else
			linkUrl		= upfORM("linkUrl",False)
		End If

		linkUrl	= FN_ONLY_PAGEURL(linkUrl)
...
'Call ResRW(strNationCode,"strNationCode")
	...
	arrParams = Array(_
		...
		Db.makeParam("@imgName",adVarWChar,adParamInput,150,imgName),_
		...
		Db.makeParam("@isViewMobile",adChar,adParamInput,1,isViewMobile),_
		Db.makeParam("@strNation",adVarChar,adParamInput,10,UCase(strNationCode)), _
		Db.makeParam("@strScontent",adVarWChar,adParamInput,200,strScontent), _

		Db.makeParam("@OUTPUT_VALUE",adVarChar,adParamOutput,10,"ERROR") _
	)
	Call Db.exec("[DKSP_POPUP_REGIST_ADMIN]",DB_PROC,arrParams,Nothing)

	OUTPUT_VALUE = arrParams(UBound(arrParams))(4)


	Select Case OUTPUT_VALUE
		Case "FINISH": Call ALERTS("정상처리되었습니다","go","popup.asp?nc="&strNationCode&"&mb="&isViewMobile)
