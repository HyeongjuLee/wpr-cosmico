...
<%
	...
	Dim styleValue,mode
		...
		Select Case mode
			Case "category1" : SQL =  "SELECT [strCatecode],[strCateName],[isView] FROM [DKT_COUNSEL_1ON1_CATEGORY] WHERE [intCateDepth] = 1 AND [strCateParent] = ? AND [strNationCode] = ? "
			Case "category2" : SQL =  "SELECT [strCatecode],[strCateName],[isView] FROM [DKT_COUNSEL_1ON1_CATEGORY] WHERE [intCateDepth] = 2 AND [strCateParent] = ? AND [strNationCode] = ? "
			Case "category3" : SQL =  "SELECT [strCatecode],[strCateName],[isView] FROM [DKT_COUNSEL_1ON1_CATEGORY] WHERE [intCateDepth] = 3 AND [strCateParent] = ? AND [strNationCode] = ? "
		End Select
...
'		PRINT SQL
		...
		arrList = Db.execRsList("DKSP_COUNSEL_1ON1_CATEGORY_LIST_AJAX_ADMIN",DB_PROC,arrParams,listLen,Nothing)
