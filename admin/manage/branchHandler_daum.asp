...
'	Select Case MODE
		Case "INSERT"
			...
			strBranchKey		= pRequestTF("strBranchKey",False)			'다음맵 추가

			strBranchMapCode = Replace(strBranchMapCode," ","")
			strBranchKey = Replace(strBranchKey," ","")
			...
			If Left(cate2,3) <> cate1 Then Call ALERTS("지사위치의 값이 올바르지 않습니다.","back","")
			...
			arrParams = Array(_
				...
				Db.makeParam("@strBranchKey",adVarChar,adParamInput,50,strBranchKey),_

				Db.makeParam("@OUTPUT_VALUE",adVarChar,adParamOutput,20,"ERROR") _
			)
			Call Db.exec("DKPA_BRANCH_INSERT_DAUM",DB_PROC,arrParams,Nothing)
		...
		Case "MODIFY"
			...
			Call Db.exec("DKPA_BRANCH_MODIFY_DAUM",DB_PROC,arrParams,Nothing)
...
'			intIDX = gRequestTF("idx",True)
			...
			Call Db.exec("DKPA_BRANCH_DELETE",DB_PROC,arrParams,Nothing)
...
Select Case OUTPUT_VALUE
	Case "ERROR"
		Call alerts("지사 정보 수정에 문제가 발생했습니다.","back","")
	Case "FINISH"
		Call ALERTS("정상처리되었습니다.","go","branch_list.asp" )
