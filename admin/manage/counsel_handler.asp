...
<%
	Select Case MODE
		Case "MODIFY"
			...
			Call Db.exec("DKPA_COUNSEL_MODIFY ",DB_PROC,arrParams,Nothing)
			OUTPUT_VALUE = arrParams(UBound(arrParams))(4)
			DB_FINISH_URL = "counsel_view.asp?nc="&LCase(strNationCode)
		Case "DELETE"
			arrParams = Array(_
				Db.makeParam("@intIDX",adInteger,adParamInput,0,intIDX), _
				Db.makeParam("@OUTPUT_VALUE",adVarChar,adParamOutput,10,"ERROR") _
			)
			Call Db.exec("DKPA_COUNSEL_DELETE",DB_PROC,arrParams,Nothing)
			OUTPUT_VALUE = arrParams(UBound(arrParams))(4)
			DB_FINISH_URL = "counsel_list.asp?nc="&LCase(strNationCode)
