...
function goCancelUBtn(uidx,tBodyID,Ri,nc) {
...
function go102Btn(uidx,tBodyID,Ri,nc) {
...
function go103Btn(uidx,tBodyID,Ri,nc) {
...
function back100Btn(uidx,tBodyID,Ri,nc) {
...
function back101Btn(uidx,tBodyID,Ri,nc) {
...
function back102Btn(uidx,tBodyID,Ri,nc) {
	if (confirm("해당 물품(그룹)을 '배송완료(배송중)' 상태로 변경하시겠습니까? \n\n확인을 누르시면 해당 물품(그룹)을 '배송완료(배송중)' 상태로 변경합니다.")) {
		loadings();
		$.ajax({
			type: "POST"
...
function backc100Btn(uidx,tBodyID,Ri,nc) {
...
//관리자주문취소 → 입금확인  상태로 변경
function backc101Btn(uidx,tBodyID,Ri,nc) {
