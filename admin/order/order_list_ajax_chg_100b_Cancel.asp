	...
	'한글
		...
		If isCSPOINTUSE_4_NORMALGOODS = "T" Then
			...
			If Left(strUserID,3) = "CS_" Then
				...
				If status ="201" Or status ="302" Then	'201 관리자주문취소/302 주문취소완료 -> 입금확인전 (backc100Btn)   (마일리지차감 -)

					If CS_USED_MILEAGE > 0 Then
						'Call Db.beginTrans(Nothing)
						SQL = "INSERT INTO [tbl_Member_Mileage] ("
						SQL = SQL & "[T_Time],[mbid],[mbid2],[M_Name],[MinusValue],[MinusKind],[Minus_OrderNumber],[User_id],[ETC1]"
						SQL = SQL & " ) VALUES ( "
						SQL = SQL & " ?,?,?,?,?,?,?,?,? "
						SQL = SQL & " )"
						arrParams = Array(_
							...
							Db.makeParam("@MinusKind",adVarchar,adParamInput,3,"12"), _
							...
							Db.makeParam("@ETC1",adVarchar,adParamInput,300,"SHOP관리자 "&status&"->100 (-)("&DK_MEMBER_ID&")") _
						)
		...
		Call Db.exec("DKPA_ORDER_CHG_100B_CANCEL",DB_PROC,arrParams,Nothing)
