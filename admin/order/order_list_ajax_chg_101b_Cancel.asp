	...
	'한글
		...
		If isCSPOINTUSE_4_NORMALGOODS = "T" Then
			...
			If Left(strUserID,3) = "CS_" Then
				...
				If status ="201" Or status ="302" Then	'201 관리자주문취소/302 주문취소완료 -> 입금확인전 (backc101Btn)   (마일리지차감 -)
					...
					If CS_USED_MILEAGE > 0 Then
						...
						arrParams = Array(_
							...
							Db.makeParam("@ETC1",adVarchar,adParamInput,300,"SHOP관리자 "&status&"->101 (-)("&DK_MEMBER_ID&")") _
						)
		...
		Call Db.exec("DKPA_ORDER_CHG_101B_CANCEL",DB_PROC,arrParams,Nothing)
