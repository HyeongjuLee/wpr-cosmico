...
' ===================================================================
	arrParams = Array( _
		...
		Db.makeParam("@STATUS",adChar,adParamInput,3,"101"), _
		...
		<table <%=tableatt%> class="width100 search">
			...
			<!-- <tr>
				...
				<td>
					...
					<label><input type="radio" name="memberType" value="S" class="input_chk input_chk_s" <%=isChecked(memberType,"S")%> />지점관리자그룹</label>
					<label><input type="radio" name="memberType" value="O" class="input_chk input_chk_s" <%=isChecked(memberType,"O")%> />오퍼레이터그룹</label>
				...
				<td>
					...
					<input type="submit" value="검색" class="input_submit design1" />
					<a href="<%=Request.ServerVariables("SCRIPT_NAME")%>" class="a_submit design2">검색초기화</a>
				</td>
			</tr>
		</table>
	</form>
</div>
<div id="member" class="member_list">
	...
	'			arrList(5,i)
				...
				PRINT "	<td class=""tcenter"">" & CallMemGroup(arrList(12,i)) &"</td>" &VbCrlf
				PRINT "	<td class=""tcenter""><input type=""button"" class=""a_submit design2"" value=""수정"" onclick=""goMemberInfo('"&arrList(3,i)&"')"" /></td>" &VbCrlf
				PRINT "</tr>" & VbCrlf
			Next
		Else
			PRINT "<tr>"
