...
''	Dim C_NAME2				: C_NAME2			= Trim(Request.Form("C_NAME2"))
	Dim bankidx				: bankidx			= pRequestTF("bankidx",True)
	Dim bankingName			: bankingName		= pRequestTF("bankingName",True)
...
'♠ GO_BACK_ADDR 주문페이지로 보내기 E♠
	...
	If usePoint2 = Null Or usePoint2 = "" Then usePoint = 0
	...
	'◆ 포인트 사용 시
		'▣총출금액 확인
		If (CDbl(usePoint) + CDbl(usePoint2)) > CDbl(MILEAGE_TOTAL) Then
...
'	Call ResRW(CDbl(usePoint),"usePoint")
...
<%
	...
	arrParams = Array(_
		...
		Db.makeParam("@payBankDate",adVarChar,adParamInput,50,v_C_Etc), _
...
<%
	...
	'■전체상품 가격 변경■
		...
		Call Db.exec(SQL11,DB_TEXT,arrParams11,DB3)
