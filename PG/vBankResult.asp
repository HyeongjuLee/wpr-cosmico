...
'IF PG_IP = "203.238.37" THEN  'PG에서 보냈는지 IP로 체크
...
'   이부분에 로그파일 경로를 수정해주세요.
	...
	Dim LogPath : LogPath = Server.MapPath ("/logings/inicis_vBank/result_") & Replace(Date(),"-","") & ".log"
...
'	If Left(NO_OID,2) = "MT" Then			'CS-1 주문 가상계좌결제
'		arrParams = Array(_
'			Db.makeParam("@NO_TID",adVarChar,adParamInput,50,NO_TID), _
'			Db.makeParam("@NO_OID",adVarChar,adParamInput,20,NO_OID), _
'			Db.makeParam("@NO_VACCT",adVarChar,adParamInput,50,NO_VACCT), _
'			Db.makeParam("@OUTPUT_VALUE",adVarChar,adParamOutput,10,"ERROR") _
'		)
'		Call Db.exec("HJP_ORDER_VBANK_RESULT_CS",DB_PROC,arrParams,DB3)
'
'	Else
		arrParams = Array(_
			Db.makeParam("@NO_TID",adVarChar,adParamInput,50,NO_TID), _
		...
		Call Db.exec("DKP_ORDER_VBANK_RESULT",DB_PROC,arrParams,Nothing)
'	End If
	...
	Select Case OUTPUT_VALUE
		Case "FINISH" : Response.WRITE "OK"
		Case "ERROR" : Response.WRITE "ERROR"
		Case Else : Response.WRITE "UNDEFINED"
	End Select
