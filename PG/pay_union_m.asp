...
function orderSubmit(f) {
	...
	*/
	...
	/* 오션트리 */
	if (chkEmpty(f.State_Code)) {
		alert("<%=LNG_JS_STATE%>");
		f.State_Code.focus();
		doubleSubmit = false;
		return false;
	}
	if (chkEmpty(f.takeState_Code)) {
		alert("<%=LNG_JS_STATE%>!");
		f.takeState_Code.focus();
		doubleSubmit = false;
		return false;
	}
	if (f.takeState_CodeCheckTF.value == 'F'){
		alert("<%=LNG_JS_STATE_CONFIRM%>");
...
function infoCopy(item) {
	...
	if (item.checked) {
		...
		f.takeState_Code.value = f.State_Code.value;
	}
	else {
		f.takeName.value = '';
		f.takeTel.value = '';
...
function calcSettlePrice() {
	...
	//SP포인트
	if (f.useCmoney2) {
		useCmoney2 = (f.useCmoney2.disabled || chkEmpty(f.useCmoney2)) ? 0 : parseInt(stripComma(f.useCmoney2.value), 10);
	}
...
function openzip_mjp2EN(target) {
	openPopup("/m/common/pop_ZipCode_JP2EN.asp?target="+target, "Zipcodes");		//모바일 일본주소(영문)
}
