	...
	<head>
		...
		<script type="text/javascript">
			function payProc(rtncode, rtnmsg, fdtid){
				//alert(rtncode);
				if(rtncode != ""){														//// #2 결제 인증값 발생시(인증성공 : 0000)

					parent.result(rtncode, rtnmsg, fdtid);								//// start.html function result 함수실행

				}else{

					try{ document.fdcall.PAYDATA.value = parent.document.fdpay.PAYDATA.value; }catch (e) { document.fdcall.PAYDATA.value = ""; }
					...
					frm.action = "https://testpg.firstpay.co.kr/jsp/main.jsp";			//// #1 결제창 호출(TEST)
					//frm.action = "https://pg.firstpay.co.kr/jsp/main.jsp";			//// #1 결제창 호출(REAL)
					frm.submit();
				}
			}
		</script>
