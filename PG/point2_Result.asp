...
<%
	...
	Dim paykind				: paykind			= Trim(pRequestTF("paykind",False))
	Dim ordersNumber		: ordersNumber		= Trim(pRequestTF("OrdNo",False))
...
' 개별 변수 받아오기
	...
	Dim strMob				: strMob			= Trim(pRequestTF("strMobile",False))
	...
	Dim takeMob				: takeMob			= Trim(pRequestTF("takeMob",False))
	...
	'배송비 관련
	Dim ori_price			: ori_price			= Trim(pRequestTF("ori_price",False))				'totalPrice ori			추가
	Dim ori_delivery		: ori_delivery		= Trim(pRequestTF("ori_delivery",False))			'totalPrice ori			추가
	...
	Dim totalOptionPrice2	: totalOptionPrice2 = Trim(pRequestTF("totalOptionPrice2",False))		'goodsOPTcost
	...
	Dim usePoint			: usePoint			= 0			'Trim(pRequestTF("useCmoney",False))
	Dim usePoint2			: usePoint2			= Trim(pRequestTF("useCmoney2",False))			'▶SP포인트		InputMileage2		InputMile_S
	...
	Dim orderMode			: orderMode			= Trim(pRequestTF("orderMode",False))	'mobile(SHOP /MOBILE주문 통합)

	Dim TOTAL_SP_POINTUSE_MAX	: TOTAL_SP_POINTUSE_MAX	= Trim(pRequestTF("TOTAL_SP_POINTUSE_MAX",False))	'▶ 최대 S포인트사용가능 금액
...
'	Call ResRw(CDbl(usePoint) + CDbl(usePoint2),"포인트합계")

	'If CDbl(ori_price) <> (CDbl(usePoint) + CDbl(usePoint2)) Then Call ALERTS("총결제금액과 포인트사용금액이 다릅니다!","GO",GO_BACK_ADDR)
	'If CDbl(usePoint2)  < CDbl(1)			  Then Call ALERTS("S포인트가 입력되지 않았습니다.","GO",GO_BACK_ADDR)
	'If CDbl(usePoint2) > CDbl(MILEAGE2_TOTAL) Then Call ALERTS("사용할수 있는 S포인트를 초과하였습니다.","GO",GO_BACK_ADDR)
	'If CDbl(usePoint2) > CDbl(TOTAL_SP_POINTUSE_MAX) Then Call ALERTS("결제가능한 S포인트보다 많이 입력하셨습니다.","GO",GO_BACK_ADDR)
	...
	If CDbl(usePoint2)  < CDbl(1)			  Then Call ALERTS(LNG_JS_POINT2,"GO",GO_BACK_ADDR)
	If CDbl(usePoint2) > CDbl(MILEAGE2_TOTAL) Then Call ALERTS(LNG_JS_POINT2_EXCEEDED,"GO",GO_BACK_ADDR)
	If CDbl(usePoint2) > CDbl(TOTAL_SP_POINTUSE_MAX) Then Call ALERTS(LNG_JS_POINT2_EXCEEDED&"!","GO",GO_BACK_ADDR)
...
'	If webproIP="T" Then
'		Call ResRw(usePoint,"usePoint - CP")
'		Call ResRw(usePoint2,"usePoint2 - SP")
'		Call ResRw(totalPrice,"totalPrice")
...
'	response.End
	...
	'▣ 웹주문번호 중복체크(2018-10-30~)
		...
		If CDbl(ORDER_DUP_CNT) > 0  Then Call ALERTS(LNG_JS_UNUSUAL_ORDERNUMBER,"GO",GO_BACK_ADDR)
		...
		For c = 0 To listLenC1
			...
			'◈ 상품 정보 확인 S
				...
				Else
					...
					Call ALERTS(LNG_SHOP_ORDER_DIRECT_05&"-"&DKRSC1_GoodIDX,"GO",GO_BACK_ADDR)
				End If
				Call closeRS(DKRSC2)
	...
	'◆ #7. SHOP 주문 임시테이블 정보 UPDATE S									'▶[totalPoint],[usePoint],[usePoint2] 추가
		arrParams = Array(_
			Db.makeParam("@intIDX",adInteger,adParamInput,0,OIDX), _
			Db.makeParam("@OrderNum",adVarChar,adParamInput,20,orderNum), _
...
' 데이터 저장
	Call Db.beginTrans(Nothing)
		...
		' 은행정보 가져오기 + CS에 은행코드 넘기기
			SQL = "SELECT A.[intIDX],A.[BankName],A.[BankNumber],A.[BankOwner],A.[isUse],B.[BankCode]"
			SQL = SQL & " FROM [DK_BANK] AS A"
			SQL = SQL & " JOIN [DK_BANK_CODE] AS B ON B.[BankName] = A.[BankName]"
	...
	'	If DKCONF_SITE_ENC = "T" AND DK_MEMBER_TYPE <> "COMPANY" Then
		...
		If infoChg = "T" And (DK_MEMBER_TYPE = "MEMBER" Or DK_MEMBER_TYPE = "ADMIN") Then
			...
			arrParams = Array(_
				Db.makeParam("@strMobile",adVarChar,adParamInput,150,strMob), _
	...
	If DK_MEMBER_TYPE = "COMPANY" Then
		SQL_N = "SELECT [Na_Code] FROM [tbl_Memberinfo] WHERE [mbid] = ? AND [mbid2] = ?"
		arrParams_N = Array(_
	...
	If DK_MEMBER_TYPE = "COMPANY" Then
			...
			If CSGoodCnt > 0 Then
				...
				arrParams2 = Array(_
					...
					Db.makeParam("@orderType",adVarChar,adParamInput,20,v_SellCode),_
					Db.makeParam("@payType",adVarChar,adParamInput,20,"point2"),_
					Db.makeParam("@payBankCode",adVarWChar,adParamInput,50,DKRS_BANK_BankCode),_
					Db.makeParam("@payBankAccNum",adVarWChar,adParamInput,100,DKRS_BANK_BankNumber),_
					...
					For i = 0 To listLen3
						...
						If DKRS4_isCSGoods = "T" Then
							...
							If Not DKRS6.BOF And Not DKRS6.EOF Then
								...
								DKRS6_Goods_Sort	= DKRS6("Goods_Sort")		'자담인 상품군
							End If
							Call closeRs(DKRS6)
