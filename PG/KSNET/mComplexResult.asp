...
'▣개별 변수 받아오기 (쇼핑몰 기본정보) S
	...
	Dim ori_delivery		: ori_delivery		= Trim(pRequestTF("ori_delivery",False))	'추가
...
<%
	...
	'♠결제정보 DB입력
		'KSNET 다카드
		'strPGFunc.asp  API -> mCardPay.asp

		oXmlParam_P = ""
		oXmlParam_P = oXmlParam_P & "TX_KSNET_TID="&TX_KSNET_TID_KEYIN
		oXmlParam_P = oXmlParam_P & "&OrderNum="&OrderNum&"_"&m
		oXmlParam_P = oXmlParam_P & "&strName="&strName
		oXmlParam_P = oXmlParam_P & "&strEmail="&strEmail
		oXmlParam_P = oXmlParam_P & "&GoodsName="&GoodsName
		oXmlParam_P = oXmlParam_P & "&strMobile="&strMobile
		oXmlParam_P = oXmlParam_P & "&CardNo="&CardNo
		oXmlParam_P = oXmlParam_P & "&cardYYMM="&cardYYMM
		oXmlParam_P = oXmlParam_P & "&Quotabase="&Quotabase
		oXmlParam_P = oXmlParam_P & "&CardPrice="&CardPrice
		oXmlParam_P = oXmlParam_P & "&CardPass="&CardPass
		oXmlParam_P = oXmlParam_P & "&CARDAUTH_Input="&CARDAUTH_Input

		Call PG_KSNET_PAY_M(oXmlParam_P, GO_BACK_ADDR, R_rStatus, R_rAuthNo, R_rMessage1, R_rMessage2, R_rTransactionNo, R_rInstallment, R_rIssCode, R_rTradeDate, R_StoreId)

		'KSNET 뒤에공백 붙어서 옴! 없애주자!!! ㅡㅜ
		rStatus			= Trim(R_rStatus)
		rAuthNo			= Trim(R_rAuthNo)
		rMessage1		= Trim(R_rMessage1)
		rMessage2		= Trim(R_rMessage2)
		rTransactionNo	= Trim(R_rTransactionNo)
		rInstallment	= Trim(R_rInstallment)
		rIssCode		= Trim(R_rIssCode)
		rTradeDate		= Trim(R_rTradeDate)
		StoreId		  	= Trim(R_StoreId)

		'Call ResRW(rStatus&"*"		,		"rStatus		")
		'Call ResRW(rAuthNo&"*"		,		"rAuthNo		")
		'Call ResRW(rMessage1&"*"	,		"rMessage1	")
		'Call ResRW(rMessage2&"*"	,		"rMessage2	")
		'Call ResRW(rTransactionNo&"*",		"rTransactionNo")
		'Call ResRW(rInstallment&"*",		"rInstallment")
		'Call ResRW(rIssCode&"*"	,		"rIssCode	")
		'Call ResRW(rTradeDate&"*"	,		"rTradeDate	")
		'Call ResRW(StoreId&"*"		  ,		"StoreId		  ")
		'Response.end
%>
...
<%
		If rStatus = "O" Then
			...
			result_cd	= "0000"					'응답코드
			result_msg	= rMessage1 &"_"& rMessage2


			PGorderNum		= rTransactionNo		'거래번호(KSNET)
			...
			PGinstallment	= rInstallment	'quotabase				'할부기간
			...
			Dim Sfile3 : Set  Sfile3 = Fso3.OpenTextFile(LogPath3,8,true)
				Sfile3.WriteLine ""
				Sfile3.WriteLine "orderNum_Fso3	: " & OrderNum&"_"&m
