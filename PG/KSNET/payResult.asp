...
<!-- #include file="KSPayWebHost.inc.asp" -->
...
'▣개별 변수 받아오기 (쇼핑몰 기본정보) S
	...
	Dim ori_delivery		: ori_delivery		= pRequestTF("ori_delivery",False)	'추가
...
<%
	...
	if kspay_send_msg("1") then	'현재 결제대기 상태이며 kspay_send_msg("1")을 호출하셔야 결제가 처리됩니다.
		...
		cardno		= kspay_get_value("cardno")			'cardno추가(KSNET 기본적으로 제공X)
...
<%
	...
	'결제정보 DB입력
		...
		PGCardNum	= cardno		'카드번호(KSNET 기본적으로 제공X)
		...
		'★금액 위변조 확인
		If CDbl(CStr(amt)) <>  CDbl(CStr(totalPrice)) Then
			ThisMsg		= "금액변조!"
