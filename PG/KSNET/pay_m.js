...
function orderSubmit(f) {
	...
	if (chkEmpty(f.strMobile)) {
		...
		f.sndMobile.value =  f.strMobile.value;
	}
	...
	if (f.gopaymethod.value == 'CardAPI')
	{
		...
		$("input[type=submit]").attr("disabled",true);			//double submit check
		f.target = "_self";
		//f.action = "/PG/KSNET/CardResult_API.asp";
		f.action = "/PG/KSNET/payResult_KeyIn.asp";
	}
	...
	if (f.gopaymethod.value == 'vBank')
	{
		...
		$("input[type=submit]").attr("disabled",true);
		f.target = "_self";
		f.action = "/PG/KSNET/vBankRequest.asp";
	...
	if (f.gopaymethod.value == 'Card')
	{
		...
		//웹프로인자값 추가S
		f.ECH_paykind.value				= f.paykind.value;
		f.ECH_OrdNo.value				= f.OrdNo.value;
		f.ECH_cuidx.value				= f.cuidx.value;
		f.ECH_gopaymethod.value			= f.gopaymethod.value;
		f.ECH_orderMode.value			= f.orderMode.value;
		f.ECH_strName.value				= f.strName.value;
		f.ECH_strTel.value				= f.strTel.value;
		f.ECH_strMobile.value			= f.strMobile.value;
		f.ECH_strEmail.value			= f.strEmail.value;
		f.ECH_strZip.value				= f.strZip.value;
		f.ECH_strADDR1.value			= f.strADDR1.value;
		f.ECH_strADDR2.value			= f.strADDR2.value;
		f.ECH_takeName.value			= f.takeName.value;
		f.ECH_takeTel.value				= f.takeTel.value;
		f.ECH_takeMobile.value			= f.takeMobile.value;
		f.ECH_takeZip.value				= f.takeZip.value;
		f.ECH_takeADDR1.value			= f.takeADDR1.value;
		f.ECH_takeADDR2.value			= f.takeADDR2.value;
		f.ECH_ori_price.value			= f.ori_price.value;
		f.ECH_ori_delivery.value		= f.ori_delivery.value;			//추가
		f.ECH_totalPrice.value			= f.totalPrice.value;
		f.ECH_totalDelivery.value		= f.totalDelivery.value;
		f.ECH_DeliveryFeeType.value		= f.DeliveryFeeType.value;
		f.ECH_GoodsPrice.value			= f.GoodsPrice.value;
		f.ECH_totalOptionPrice.value	= f.totalOptionPrice.value;
		f.ECH_totalOptionPrice2.value	= f.totalOptionPrice2.value;
		f.ECH_totalPoint.value			= f.totalPoint.value;
		f.ECH_usePoint.value			= f.useCmoney.value;
		f.ECH_usePoint2.value			= f.useCmoney2.value;
		f.ECH_totalVotePoint.value		= f.totalVotePoint.value;
		f.ECH_GoodsName.value			= f.GoodsName.value;
		f.ECH_TOTAL_POINTUSE_MAX.value	= f.TOTAL_POINTUSE_MAX.value;
		f.ECH_orderMemo.value			= f.orderMemo.value;
		f.ECH_ea.value					= f.ea.value;
		f.ECH_GoodIDXs.value			= f.GoodIDXs.value;
		f.ECH_strOptions.value			= f.strOptions.value;
		f.ECH_OIDX.value				= f.OIDX.value;
		f.ECH_isDirect.value			= f.isDirect.value;
		f.ECH_GoodIDX.value				= f.GoodIDX.value;
		f.ECH_CSGoodCnt.value			= f.CSGoodCnt.value;
		f.ECH_isSpecialSell.value		= f.isSpecialSell.value;
		f.ECH_v_SellCode.value			= f.v_SellCode.value;
		f.ECH_SalesCenter.value			= f.SalesCenter.value;
		f.ECH_DtoD.value				= f.DtoD.value;

		/*
		f.ECHA.value				= (f.strName.value);					//테스트001
		f.ECHB.value				= (f.strADDR1.value);					//서울 구로구 디지털로33길 11 (구로동, 에이스테크노타워8차)
		f.ECHC.value				= (f.DeliveryFeeType.value);			//선결제
		f.ECHD.value				= (f.strEmail.value);					//abc@abc.te
		*/

		//웹프로인자값 추가E

 		//f.sndReply.value = getLocalUrl("kspay_wh_result.asp") ;
		f.sndReply.value = f.sndReply.value+"/PG/KSNET/payResult_m.asp";
		...
		//f.target = '_blank';
		//f.action ='http://kspay.ksnet.to/store/KSPayMobileV1.4/KSPayPWeb.jsp';
		f.action ='https://kspay.ksnet.to/store/KSPayMobileV1.4/KSPayPWeb.jsp';
	...
	if (f.gopaymethod.value == 'mComplex')
	{
		...
		*/
		...
		f.action = "/PG/KSNET/mComplexResult.asp";
...
function checkpayType() {
	...
	if (payKind == 'Card')
	{
		...
		$("#sndPaymethod").val("1000000000");		//KSNET CARD("1000000000")
			$("input[name=useCmoney]").removeClass("readonly").attr("readonly",false).val(0);
			$("#pointTXT").text('');
			calcSettlePrice();
	}
