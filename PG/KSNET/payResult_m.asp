...
'▣개별 변수 받아오기 (쇼핑몰 기본정보) S
	...
	Dim strName				: strName			= FN_URLDecode(pRequestTF("ECH_strName",True))
	Dim strTel				: strTel			= FN_URLDecode(pRequestTF("ECH_strTel",False))
	Dim strMobile			: strMobile			= FN_URLDecode(pRequestTF("ECH_strMobile",True))
	Dim strEmail			: strEmail			= FN_URLDecode(pRequestTF("ECH_strEmail",False))
	Dim strZip				: strZip			= FN_URLDecode(pRequestTF("ECH_strZip",True))
	Dim strADDR1			: strADDR1			= FN_URLDecode(pRequestTF("ECH_strADDR1",True))
	Dim strADDR2			: strADDR2			= FN_URLDecode(pRequestTF("ECH_strADDR2",True))

	Dim takeName			: takeName			= FN_URLDecode(pRequestTF("ECH_takeName",False))
	Dim takeTel				: takeTel			= FN_URLDecode(pRequestTF("ECH_takeTel",False))
	Dim takeMobile			: takeMobile		= FN_URLDecode(pRequestTF("ECH_takeMobile",False))
	Dim takeZip				: takeZip			= FN_URLDecode(pRequestTF("ECH_takeZip",False))
	Dim takeADDR1			: takeADDR1			= FN_URLDecode(pRequestTF("ECH_takeADDR1",False))
	Dim takeADDR2			: takeADDR2			= FN_URLDecode(pRequestTF("ECH_takeADDR2",False))

	'Dim infoChg				: infoChg			= pRequestTF("infoChg",False)


	Dim ori_price			: ori_price			= FN_URLDecode(pRequestTF("ECH_ori_price",True))
	Dim ori_delivery		: ori_delivery		= FN_URLDecode(pRequestTF("ECH_ori_delivery",True))		'추가
	...
	Dim totalOptionPrice	: totalOptionPrice  = FN_URLDecode(pRequestTF("ECH_totalOptionPrice",False))
	Dim totalOptionPrice2	: totalOptionPrice2 = FN_URLDecode(pRequestTF("ECH_totalOptionPrice2",False))		'goodsOPTcost
	Dim totalPoint			: totalPoint		= FN_URLDecode(pRequestTF("ECH_totalPoint",False))
	...
	Dim orderEaD			: orderEaD	= FN_URLDecode(pRequestTF("ECH_ea",False))			'◆ #5. EA
	Dim GoodIDXs			: GoodIDXs	= FN_URLDecode(pRequestTF("ECH_GoodIDXs",False))		'◆ #5. GoodIDXs
	Dim strOptions			: strOptions= FN_URLDecode(pRequestTF("ECH_strOptions",False))	'◆ #5. strOptions
...
'▣isDirect or Cart 체크 S
	isDirect		= FN_URLDecode(pRequestTF("ECH_isDirect",False))
	GoodIDX			= FN_URLDecode(pRequestTF("ECH_GoodIDX",False))
...
'▣ CS 특이사항 S
	CSGoodCnt		= FN_URLDecode(pRequestTF("ECH_CSGoodCnt",True))			'통합정보 CS상품 갯수
	isSpecialSell	= FN_URLDecode(pRequestTF("ECH_isSpecialSell",False))
	If DK_MEMBER_TYPE = "COMPANY" And CSGoodCnt > 0 Then
		v_SellCode		= FN_URLDecode(pRequestTF("ECH_v_SellCode",True))		'CS상품 구매종류
		SalesCenter		= FN_URLDecode(pRequestTF("ECH_SalesCenter",False))		'판매센터
		DtoD			= FN_URLDecode(pRequestTF("ECH_DtoD",True))				'판매센터
	Else
		v_SellCode		= ""
		SalesCenter		= ""
		DtoD			= "T"
...
<%
	...
	If rcid = "" Then
		...
		GO_CANCEL_ADDR = "/m/shop/cart_directB.asp?gidx="&gidx
...
<%
				...
				Else
					''ThisMsg = "구매성공 (모바일 일반)테스트 결제 삭제 처리!!!"
					''Call PG_KSNET_CANCEL(StoreId,PGorderNum,GoodIDX,isDirect,chgPage, ThisMsg)
