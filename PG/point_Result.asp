...
' 개별 변수 받아오기
	...
	Dim takeMob				: takeMob			= Trim(pRequestTF("takeMobile",False))
...
'	Call ResRw(CDbl(usePoint) + CDbl(usePoint2),"포인트합계")
	...
	'If CDbl(usePoint)  < CDbl(1)			  Then Call ALERTS("C포인트가 입력되지 않았습니다.","GO",GO_BACK_ADDR)
	'If CDbl(usePoint)  > CDbl(MILEAGE_TOTAL)  Then Call ALERTS("사용할수 있는 C포인트를 초과하였습니다.","GO",GO_BACK_ADDR)
	...
	If DK_MEMBER_TYPE = "COMPANY" Then
			...
			If CSGoodCnt > 0 Then
				...
				arrParams2 = Array(_
					...
					Db.makeParam("@orderType",adVarChar,adParamInput,20,v_SellCode),_
					Db.makeParam("@payType",adVarChar,adParamInput,20,"point"),_
				...
				If IsArray(arrList3) Then
					...
					Next
						...
						'■전체상품 가격 변경■
							...
							v_Etc1 = "CP :"&num2cur(usePoint)&", SP :"&num2cur(usePoint2)
