...
<%
	...
	'◆ #7. SHOP 주문 임시테이블 정보 UPDATE E
		...
		Dim LogPath : LogPath = Server.MapPath (CardLogss&"/pResult_") & Replace(Date(),"-","") & ".log"
		...
		Sfile.WriteLine "TOTAL_PV		 : " & TOTAL_PV
...
'★★★★ YESPAY(링크나인) API ★★★★
		...
		bXmlParam = bXmlParam & "&payAmount="&totalPrice									'결제할금액(*)	YESPAY
...
'		print bXmlParam
		...
		Dim LogPath2 : LogPath2 = Server.MapPath (CardLogss&"/pResult_") & Replace(Date(),"-","") & ".log"
		Dim Sfile2 : Set  Sfile2 = Fso2.OpenTextFile(LogPath2,8,true)
			...
			If bXmlStatus >= 400 And bXmlStatus <= 599 Then
				Sfile2.WriteLine "PG 연결에 문제가 발생했습니다.1 : " & resMessage
				Call ALERTS("PG 연결에 문제가 발생했습니다.1 \n\n에러메세지 : "&resMessage&"","GO",GO_BACK_ADDR)

			Else
				If LCase(resCode) <> "00" Then
					Sfile2.WriteLine "PG 연결에 문제가 발생했습니다2.1 : " & resMessage
		...
		'YESPAY
			PGorderNum	= bInfo.orderId		'거래번호
			PGCardNum	= bInfo.cardNo		'카드번호
			...
			PGAcceptNum		= bInfo.apprNo						'신용카드 승인번호
			PGinstallment	= bInfo.installment					'할부기간
			PGCardCode		= YESPAY_CARDCODE(bInfo.cardCode)	'신용카드사코드(YESPAY 치환)
			PGCardCom		= ""								'신용카드발급사
			...
			Dim LogPath3 : LogPath3 = Server.MapPath (CardLogss&"/pResult_") & Replace(Date(),"-","") & ".log"
				...
				If IsArray(arrListC3) Then
					For k = 0 To listLenC3
						...
						'상품 정보(수량) 확인
							...
							'상품정보 확인 후 취소가 필요한 경우
								If ThisCancel = "T" Then
									Call PG_YESPAY_CANCEL(PGorderNum, orderNum, totalPrice, isDirect, GoodIDX, chgPage, ThisMsg)
								End If
				...
				'=== CS 전산입력 =============================================================================================================
					...
					' CS상품 갯수체크
							If CSGoodCnt > 0 Then
								...
								'주문정보에서 CS상품을 검색한다
								SQL3 = "SELECT [GoodIDX],[OrderEa],[strOption] FROM [DK_ORDER_GOODS] WHERE [orderIDX] = ?"
								arrParams3 = Array(_
								...
								If IsArray(arrList3) Then
									For i = 0 To listLen3
										...
										arrList3_strOption		= arrList3(2,i)
										...
										If DKRS4_isCSGoods = "T" Then
											...
											SQL7 = SQL7 & " [OrderIDX],[GoodsCode],[GoodsPrice],[GoodsPV],[ea],[strOption] "
											SQL7 = SQL7 & " ) VALUES ("
											SQL7 = SQL7 & " ?,?,?,?,?,?"
											SQL7 = SQL7 & " )"
											arrParams7 = Array(_
												...
												Db.makeParam("@strOption",adVarWChar,adParamInput,800,arrList3_strOption) _
											)
									...
									Next
										...
										'전체상품 가격 체크
											...
											Dim LogPath5 : LogPath5 = Server.MapPath (CardLogss&"/pResult_") & Replace(Date(),"-","") & ".log"
