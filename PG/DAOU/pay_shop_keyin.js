...
function chgDelivery(DtoD) {
	...
	if (DtoD == 'F')
	{
		...
		$("input[name=AMOUNT]").val((price - deliFee) * 1);			//이니시스결제금액
		$("input[name=totalDelivery]").val(0);
		$("#priTXT").html("<strong>현장수령 : 0</strong> 원");
		$("#payArea").text(formatComma((price - deliFee) * 1));
	} else {
		$("input[name=totalPrice]").val(price);
		$("input[name=AMOUNT]").val(price);							//이니시스결제금액
		$("input[name=totalDelivery]").val(deliFee);
		$("#priTXT").html("<strong>"+formatComma(deliFee)+"</strong> 원");
		$("#payArea").text(formatComma(price));
	}
}
