...
function fnSubmit() {
	...
	if(trim(f.ORDERNO.value) == "" || getByteLen(f.ORDERNO.value) > 50) {
		alert("Ordernumber(ORDERNO) is empty. (최대:50byte, 현재:" + getByteLen(f.ORDERNO.value) + ")");
		return false;
	}
	//상품구분
	if(trim(f.PRODUCTTYPE.value) == "" || getByteLen(f.PRODUCTTYPE.value) > 2) {
		alert("Product type(PRODUCTTYPPE) is empty. (최대:2byte, 현재:" + getByteLen(f.PRODUCTTYPE.value) + ")");
		return false;
	}
	//과금유형
	if(trim(f.BILLTYPE.value) == "" || getByteLen(f.BILLTYPE.value) > 2) {
		alert("Payment type(BILLTYPE) is empty. (최대:2byte, 현재:" + getByteLen(f.BILLTYPE.value) + ")");
		return false;
	}
	//결제금액
	if(trim(f.AMOUNT.value) == "" || getByteLen(f.AMOUNT.value) > 10) {
		alert("Total Amount(AMOUNT) is empty. (최대:10byte, 현재:" + getByteLen(f.AMOUNT.value) + ")");
		return false;
	...
	{
		alert("The item name is empty. It is essential information.");
		return false;
	}
