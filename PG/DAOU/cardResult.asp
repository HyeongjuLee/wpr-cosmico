...
<%
	...
	'스피나 신버전 쇼핑몰구매(cs연동) (MYOFFICE, SHOP 구매) [일반] 2017-05-16 ~
				...
				'	Dim inUidx				: inUidx				= Trim(DAOU_SPLIT(15))
					...
					Select Case UCase(paykind)
						Case "CARD"
							...
							If DAOU_SETTDATE <> "" Then
								RegTime = Left(DAOU_SETTDATE,8)
							End If
...
<%
		...
		'	If DKCONF_SITE_ENC = "T" And isMACCO = "T" Then		'암호화/직판
		'	If MACCO_TF = "T" And Daou_MEMBER_TYPE = "COMPANY" And CSGoodCnt > 0 Then		'직판, 판매원, CS상품구매일때
			If DKCONF_SITE_ENC = "T" And MACCO_TF = "T" And Daou_MEMBER_TYPE = "COMPANY" And CSGoodCnt > 0 And insNUM4PassBook = "T" Then		'암호화/직판 CS상품구매일때/▣▣▣ 무통장구매시 공제번호 실시간 발생!! ▣▣▣

			' 이하 직판공제조합 신고 프로세스 DAOU_FUNCTION 의 MACCO_NUM 을 반드시 확인할 것
				'SQL = "SELECT [cpno],[M_name],[Sell_Mem_TF],[Sex],[Na_Code] FROM [tbl_memberinfo] where [mbid] = ? and [mbid2] = ?"
				SQL = "SELECT [cpno],[M_name],[Sell_Mem_TF],[Sex],[Na_Code],[BirthDay],[BirthDay_M],[BirthDay_D] FROM [tbl_memberinfo] where [mbid] = ? and [mbid2] = ?"
				arrParams = Array(_
					Db.makeParam("@MBID1",adVarChar,adParamInput,20,Daou_MEMBER_ID1), _
					Db.makeParam("@MBID2",adInteger,adParamInput,4,Daou_MEMBER_ID2) _
				)
				...
				If Not DKRS.BOF And Not DKRS.EOF Then
					CS_CPNO			= DKRS(0)
					CS_MNAME		= DKRS(1)
					CS_Sell_Mem_TF	= DKRS(2)
					CS_D_Sex		= DKRS(3)
					CS_Na_Code		= DKRS(4)
					CS_BirthDay		= DKRS(5)		'주민번호 수집불가
					CS_BirthDay_M	= DKRS(6)
					CS_BirthDay_D	= DKRS(7)
				...
				If Daou_MEMBER_TYPE = "GUEST" THEN	'▣비회원구매시 신고자이름 치환!!
					CS_MNAME = strName
				End If
			...
			'	End If
				...
				If CS_SELLER_TYPE = 2 Then '소비자회원
					CS_Birth = "999999"&"9"
				Else
					CS_Birth = CS_Birth&"9"			'성별정보 없는경우 무조건 9 (2017-03-03)
				End If
				...
				' 직판용 공제번호 체크썸
				If UCase(CS_Na_Code) = "KR" Then									'한국회원만 처리

					Select Case RS_Card_Sort_Code	'결제수단 구분코드 추가(CH : 현금(무통장입금포함) / MI : 복합결제(카드 + 현금 등))
						Case "0"
							 PAY_METHOD = "CD"	'신용카드
						Case "1"
							 PAY_METHOD = "CC"	'체크카드
					...
					'DK_MEMBER_NAME = "가나다"
					oXmlParam = ""
					oXmlParam = oXmlParam & "orderid="&OUT_ORDERNUMBER
					oXmlParam = oXmlParam & "&shopid="&MACCO_NUM
					oXmlParam = oXmlParam & "&totalmoney="&totalPrice-totalDelivery	'직판신고 : 배송비제외!
					oXmlParam = oXmlParam & "&seller_type="&CS_SELLER_TYPE			'1=판매원  2=소비자
					oXmlParam = oXmlParam & "&name="&Server.URLEncode(CS_MNAME)
					oXmlParam = oXmlParam & "&userid="&CS_Birth
					oXmlParam = oXmlParam & "&mem_id="&Daou_MEMBER_ID1&"-"&Daou_MEMBER_ID2
					oXmlParam = oXmlParam & "&merc_code="
					oXmlParam = oXmlParam & "&ctype=w"
					oXmlParam = oXmlParam & "&returntype=xml"
					oXmlParam = oXmlParam & "&pay_method="&UCase(PAY_METHOD)		'결제수단 구분코드
					...
					Set oDOM = Server.CreateObject("Microsoft.XMLDOM")
						...
						Select Case MLM_guaranteeResult
							...
							Case "N"
								...
								ALERTS_MESSAGE = "공제번호 발급이 실패되었습니다. 에러코드 : "&MLM_errorCode&"\n\nhttps://m.macco.co.kr/it/errcode/list.action 접속해서 에러코드 확인후에\n\n"&DKCONF_SITE_TITLE&" 전산담당자에게 문의해 보시기 바랍니다."
								'ALERTS_MESSAGE = "주문번호 : "&orderNum&" 의 공제번호 발급이 실패되었습니다.에러코드 : "&MLM_errorCode
