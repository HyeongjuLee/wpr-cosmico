...
//alert(f.gopaymethod.value);
	...
	if (chkEmpty(f.strTel1) || chkEmpty(f.strTel2) || chkEmpty(f.strTel3))
	{
		alert("주문자 연락처가 비어있습니다");
		f.strTel1.focus();
		return false;
	} else {
		f.TELNO1.value = f.strTel1.value +"-"+ f.strTel2.value +"-"+ f.strTel3.value;
	}
	...
	{
	/*
		if (f.bankidx.checked == false)
		{
			alert("온라인 입금시 결제 은행을 선택해주셔야합니다.1");
			f.bankidx.focus();
			return false;
		}
		if (f.bankidx[0].checked == false && f.bankidx[1].checked == false)
		{
			alert("온라인 입금시 결제 은행을 선택해주셔야합니다.2");
			f.bankidx[0].focus();
			return false;
		}
	*/
	...
	//return false;
		...
		valData += "☞"+f.takeZip.value.replaceAll('☞','');					//10
		...
		valData += "☞"+f.strOption.value.replaceAll('☞','');					//20
