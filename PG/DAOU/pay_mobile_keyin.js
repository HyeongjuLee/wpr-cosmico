...
function loadings() {
	...
	loadingBar.toggle();
	$("#top").css({"opacity":"0.7","background-color":"#fff;"});		//모방일 상단 메뉴 안보임
...
function fnSubmit() {
	...
	if (f.gopaymethod.value == 'inBank')
	{
		...
		f.action = "/PG/inbank_Result.asp";							//PG, Mob결제 통합
	}
	...
	{	
		...
		*/
		...
		document.charset = "UTF-8";
		document.getElementById("frmConfirm").acceptCharset = "utf-8";
		f.action = "/PG/DAOU/cardResult_API.asp";							//PG, Mob결제 통합
...
//alert($("input[name=DtoD]:checked").val());
		...
		//alert(document.charset);
			...
			if (f.gopaymethod.value == 'card' || f.gopaymethod.value == 'Card' ) {
				...
				fileName = "http://ssl.daoupay.com/m/vaccount/DaouVaccountMng.jsp";		//모바일 가상계좌
				DAOUPAY = window.open("", "DAOUPAY", "width=473,height=538");

			} else if (f.gopaymethod.value == 'dBank') {
				fileName = "https://ssl.daoupay.com/m/bank/DaouBankMng.jsp";			//모바일
