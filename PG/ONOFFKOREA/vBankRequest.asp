...
'▣가상계좌 결제 정보 S
	Dim vBankCode			: vBankCode			= pRequestTF("vBankCode",True)
	Dim vBankDepName		: vBankDepName		= pRequestTF("vBankDepName",True)		'입금자명
	Dim vBankDepDate		: vBankDepDate		= pRequestTF("vBankDepDate",True)		'입금만료시간




'	Call ResRw(vBankCode,"vBankCode")
'	Call ResRw(ONOFFKOREA_vBANKCODE(vBankCode),"vBankCode치환")
...
'▣ ONOFF 가상계좌 최소금액 확인
'	If CDbl(totalPrice) < 300 Then Call ALERTS("결제시 결제금액이 최소 300원 이상이어야 합니다","GO",GO_BACK_ADDR)
...
'▣ 웹주문번호 중복체크 E
	...
	'◆ #7. SHOP 주문 임시테이블 정보 UPDATE E
		...
		Dim LogPath : LogPath = Server.MapPath (CardLogss&"/vBank_") & Replace(Date(),"-","") & ".log"
...
'★★★★ ONOFFKOREA API ★★★★
		...
		oXmlParam_P = oXmlParam_P & "&ordr_idxx="&orderNum														'(필) 주문번호 orderNum
		oXmlParam_P = oXmlParam_P & "&good_name="&Left(eRegiReplace(GoodsName,"[^-가-힣a-zA-Z0-9]",""),15)		' (선) 결제 상품명
		oXmlParam_P = oXmlParam_P & "&good_mny="&totalPrice														'(필) 입금금액
		oXmlParam_P = oXmlParam_P & "&buyr_name="&Left(eRegiReplace(strName,"[^-가-힣a-zA-Z0-9]",""),15)		' (선) 구매자
		oXmlParam_P = oXmlParam_P & "&buyr_mail="&strEmail														' (선) 이메일
		oXmlParam_P = oXmlParam_P & "&buyr_tel1="&strTel														' (선) 전화번호
		oXmlParam_P = oXmlParam_P & "&buyr_tel2="&strMobile														' (선) 휴대폰번호
		oXmlParam_P = oXmlParam_P & "&ipgm_name="&Left(eRegiReplace(vBankDepName,"[^-가-힣a-zA-Z0-9]",""),15)	'(필) 입금자명
		oXmlParam_P = oXmlParam_P & "&ipgm_bank="&ONOFFKOREA_vBANKCODE(vBankCode)								'(필) 입금은행
		oXmlParam_P = oXmlParam_P & "&ipgm_date="&vBankDepDate													'(필) 입금기한일년월일시분초


		'oXmlURL_P = "https://store.onoffkorea.co.kr/v_account/req_v_account_dev.php"		'테스트결제
		oXmlURL_P = "https://store.onoffkorea.co.kr/v_account/req_v_account.php"			'실결제
		...
		Dim LogPath2 : LogPath2 = Server.MapPath (CardLogss&"/vBank_") & Replace(Date(),"-","") & ".log"
		Dim Sfile2 : Set  Sfile2 = Fso2.OpenTextFile(LogPath2,8,true)

			Sfile2.WriteLine "--- vBank Request (ISSUE)---------------------"
			...
			Select Case oXmlStatus_P
					...
					Set objMsXmlDom = Server.CreateObject("microsoft.XMLDOM")
						...
						If result_cd <> "0000" Then
							ALERT_MSG = "result_cd : "&result_cd&","&ALERT_BR&"result_msg : "&result_msg&""
								...
								Sfile2.WriteLine "---------------------------------------------"
							Call ALERTS("가상계좌 발급 도중 오류가 발생했습니다. ["&ALERT_MSG&"]","GO",GO_BACK_ADDR)
							Response.End
						End If

						ordr_idxx	= "" & Trim(objMsXmlDom.getElementsByTagName("ordr_idxx").Item(0).Text)			'가맹점 주문번호
						tno			= "" & Trim(objMsXmlDom.getElementsByTagName("tno").Item(0).Text)				'가상계좌 거래번호
						amount		= "" & Trim(objMsXmlDom.getElementsByTagName("amount").Item(0).Text)			'입금금액
						app_time	= "" & Trim(objMsXmlDom.getElementsByTagName("app_time").Item(0).Text)			'승인 시간
						va_date		= "" & Trim(objMsXmlDom.getElementsByTagName("va_date").Item(0).Text)			'입금마감시간
						account		= "" & Trim(objMsXmlDom.getElementsByTagName("account").Item(0).Text)			'가상계좌번호
						bankcode	= "" & Trim(objMsXmlDom.getElementsByTagName("bankcode").Item(0).Text)			'가상계좌은행코드
						bankname	= "" & Trim(objMsXmlDom.getElementsByTagName("bankname").Item(0).Text)			'가상계좌은행
						depositor	= "" & Trim(objMsXmlDom.getElementsByTagName("depositor").Item(0).Text)			'가상계좌주명

					Set objMsXmlDom  = Nothing

					Sfile2.WriteLine "ordr_idxx	: "	& ordr_idxx
					Sfile2.WriteLine "tno		: "	& tno
					Sfile2.WriteLine "amount	: "	& amount
					Sfile2.WriteLine "app_time	: "	& app_time
					Sfile2.WriteLine "va_date	: "	& va_date
					Sfile2.WriteLine "account	: "	& account
					Sfile2.WriteLine "bankcode	: "	& bankcode
					Sfile2.WriteLine "bankname	: "	& bankname
					Sfile2.WriteLine "depositor	: "	& depositor
					Sfile2.WriteLine "---------------------------------------------"
				...
				Case Else
					ALERT_MSG = "ErrorCode : "&oXmlStatus_P&""
						...
						Sfile2.WriteLine "ALERT_MSG	: " & ALERT_MSG
						Sfile2.WriteLine "---------------------------------------------"
					Call ALERTS("가상계좌 발급 도중 오류가 발생했습니다2. ["&ALERT_MSG&"]","GO",GO_BACK_ADDR)
					Response.End
...
'Response.end
	...
	'결제정보 DB입력
		...
		'ONOFFKOREA
			...
			PGCardCom		= ""
			PGAcceptDate	= ""
			PGCOMPANY		= "ONOFFKOREA"

			'** 은행 정보 치환 E
			vBankCode			= vBankCode			'가상계좌 은행코드
			vBankName			= bankname			'가상계좌 은행명
			vBankDepName		= vBankDepName		'가상계좌 입금자명
			vBankAccNum			= account			'가상계좌 입금계좌번호
			vBankRecvName		= depositor			'가상계좌 수취인명

			vBankDepDate		= va_date			'가상계좌 입금 만료일
			vBankAmt			= amount			'가상계좌 결제금액
			vBankSetDate		= ""				'가상계좌 결제일
			vBankTRX			= tno				'가상계좌 거래번호
			vBankStatus			= "ISSUE"			'가상계좌 상태
		...
		If result_cd = "0000" Then
				...
				'입력 시작
					...
					SQL = SQL & " ,[vBankCode],[vBankName],[vBankDepName],[vBankAccNum],[vBankRecvName]"
					SQL = SQL & " ,[vBankDepDate],[vBankAmt],[vBankSetDate],[vBankTRX],[vBankStatus]"
					...
					arrParams = Array( _
						...
						Db.makeParam("@vBankDepName",adVarWchar,adParamInput,100,vBankDepName), _
						...
						Db.makeParam("@vBankAmt",adInteger,adParamInput,4,vBankAmt), _
						...
						Db.makeParam("@vBankStatus",adVarchar,adParamInput,20,vBankStatus), _
				...
				'=== CS 전산입력 =============================================================================================================
					If DK_MEMBER_TYPE = "COMPANY" Then
							...
							If CSGoodCnt > 0 Then
								...
								SQL2 = SQL2 & " ,[vBankCode],[vBankName],[vBankDepName],[vBankAccNum],[vBankRecvName]"
								SQL2 = SQL2 & " ,[vBankDepDate],[vBankAmt],[vBankSetDate],[vBankTRX],[vBankStatus]"
								SQL2 = SQL2 & " ,[PGIDS]"
								...
								arrParams2 = Array(_
									...
									Db.makeParam("@vBankTRX",adVarchar,adParamInput,20,vBankTRX), _
									Db.makeParam("@vBankStatus",adVarchar,adParamInput,20,vBankStatus), _
									Db.makeParam("@PGIDS",adVarChar,adParamInput,50,PGIDS), _
								...
								If IsArray(arrList3) Then
									...
									Next
										...
										'결제금액의 변조확인
											If CDbl(CStr(WEBDB_TOTAL_PRICE)) <> CDbl(CStr(DKCS_TOTAL_PRICE)) Then Call alerts("결제금액의 변조되었습니다.01","GO",GO_BACK_ADDR)
											If CDbl(CStr(totalPrice)) <> CDbl(CStr(DKCS_TOTAL_PRICE)) Then Call alerts("결제금액의 변조되었습니다.02","GO",GO_BACK_ADDR)
											...
											v_Etc1 = PGCOMPANY&"/ #웹가상계좌/"&vBankTRX&"/"&orderNum&"_("&DK_MEMBER_ID1&"-"&DK_MEMBER_ID2&")"	'거래번호 / 주문번호
											...
											arrParams = Array(_
												...
												Db.makeParam("@v_C_Code",adVarChar,adParamInput,50,vBankCode),_
												Db.makeParam("@v_C_Number1",adVarChar,adParamInput,100,vBankAccNum),_
												Db.makeParam("@v_C_Number2",adVarChar,adParamInput,100,""),_
												Db.makeParam("@v_C_Name1",adVarWChar,adParamInput,50,vBankName),_
												Db.makeParam("@v_C_Name2",adVarWChar,adParamInput,50,vBankDepName),_
											...
											'◆ #10. (DB3)임시주문정보에서 CS주문번호를 삽입한다. S
											...
											'◆ #10. (DB3)임시주문정보에서 CS주문번호를 삽입한다. E
