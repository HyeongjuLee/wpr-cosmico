...
function orderSubmit(f) {
	...
	if (f.gopaymethod.value == 'inBank')
	{
		...
		var receiptInfoTF = $("input[name=receiptInfoTF]:checked").val();

		if (receiptInfoTF == 'T') {
			var receiptType = $("input[name=receiptType]:checked").val();
			if (receiptType == 'P') {
				var receiptPType = $("select[name=receiptPType]").val();
				switch (receiptPType) {
					case "hpNum" : {
						for (var i=1; i<=3; i++) {
							objItem = eval('f.recriptHp'+i);
							if (chkEmpty(objItem)) {
								alert("현금영수증을 발급받을 핸드폰 번호를 입력해 주세요.");
								objItem.focus();
								doubleSubmit = false;
								return false;
							}
					...
					case "ssNum" : {
						for (var i=1; i<=2; i++) {
							objItem = eval('f.recriptSn'+i);
							if (chkEmpty(objItem)) {
								alert("현금영수증을 발급받을 주민등록번호를 입력해 주세요.");
								objItem.focus();
								doubleSubmit = false;
								return false;
							}
						}
						//if (!checkSSHs(f.recriptSn1.value, f.recriptSn2.value)) {
						if (!checkSSHs(f.recriptSn1, f.recriptSn2)) {
							doubleSubmit = false;
							return false;
						}
						break;
					...
					case "CdNum" : {
						for (var i=1; i<=4; i++) {
							objItem = eval('f.recriptCd'+i);
							if (chkEmpty(objItem)) {
								alert("현금영수증을 발급받을 현금영수증카드번호를 입력해 주세요.");
								objItem.focus();
								doubleSubmit = false;
								return false;
							}
					...
					default : {
						alert("발급받으실 현금영수증번호의 종류를 선택해주세요.");
						doubleSubmit = false;
						return false;

					}
				}
			} else if (receiptType == 'C') {
				var recriptCnS = '';
				for (var i=1; i<=3; i++) {
					objItem = eval('f.recriptCn'+i);
					if (chkEmpty(objItem)) {
						alert("현금영수증을 발급받을 사업자번호를 입력해 주세요.");
						objItem.focus();
						doubleSubmit = false;
						return false;
					}
					recriptCnS += objItem.value;
				}
				//console.log(recriptCnS);
				if (recriptCnS.length == 10) {
					//사업자등록번호 유효성
					if (chkBusinessNo(recriptCnS) == false) {
	...
	//현금결제(렌탈신청)
	if (f.gopaymethod.value == 'inCash')
	{

		var rental_confirm = ''
		if (parseInt(f.MEMBER_RENTAL_ORDER_CNT.value, 10) > 0 )	{
			rental_confirm = '렌탈 신청한 이력이 있습니다. 다시 신청하는 것이 맞습니까?'
		} else {
			rental_confirm = '렌탈 신청을 진행하시겠습니까?'
		}

		if (confirm(rental_confirm)) {
			loadings();
			$("input[type=submit]").attr("disabled",true);
			f.target = "_self";
			f.action = "/PG/inCash_Result.asp";
		} else {
			doubleSubmit = false;
			return false;
		}
	...
	if (f.gopaymethod.value == 'Card')
	{
		...
		f.target = "_self";
		f.action = "/PG/ONOFFKOREA/CardResult.asp";
	...
	if (f.gopaymethod.value == 'vBank')
	{
		...
		$("input[type=submit]").attr("disabled",true);
		f.target = "_self";
		f.action = "/PG/ONOFFKOREA/vBankRequest.asp";
	...
	if (f.gopaymethod.value == 'mComplex')
	{
		...
		*/
		...
		f.action = "/PG/ONOFFKOREA/mComplexResult.asp";
...
function calcSettlePrice() {
	...
	$("#lastTXT, #mComplexTotal_TXT").text(formatComma(settlePrice));		//복합결제 포인트 사용시 추가 mComplexTotal_TXT
...
function checkpayType() {
	...
	if (payKind == 'Card')
	{
		$("#CardInfo").css({"display":"block"});
		$("#BankInfo").css({"display":"none"});
		$("#CashReceiptInfo").css({"display":"none"});
	...
	if (payKind == 'inBank')
	{
		$("#CardInfo").css({"display":"none"});
		$("#BankInfo").css({"display":"block"});
			$("#CashReceiptInfo").css({"display":"block"});		//현금영수증
		$("#vBankInfo").css({"display":"none"});
		$("#mComplexInfo").css({"display":"none"});
		//$("#TOTAL_PV").text($("#ori_TOTAL_PV").val());
