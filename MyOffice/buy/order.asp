...
<link rel="stylesheet" href="/myoffice/css/shop_style.css?v0.1" />
<%
...
Select Case UCase(DK_MEMBER_NATIONCODE)
	Case "KR"
		...
		<script type="text/javascript" src="/shop/order_mComplex.js?v1"></script>
		<%
		'	Case "ONOFFKOREA"
		'		BODYLOAD = ""
		...
		<%
			Case "KSNET"
				BODYLOAD = ""
				PRINT "<script type=""text/javascript"" src=""/PG/KSNET/pay_cs.js?v1.3""></script>"
		...
		<script type="text/javascript" src="/shop/order_mComplex.js?v0"></script>
...
<body onLoad="<%=BODYLOAD%>"  <%=mouseLock()%>>
...
<div id="cart">
	<div class="cleft width100 cart_list">
		...
		<%
			...
			%>
				<tr>
					<td class="tcenter"><%=arrList_CSGoodsCode%></td>
					<td class="">
						<p class="goodsName"><%=backword(arrList_GoodsName)%></p>
						<p class=""><%=arr_Goods_Sort_TXT%></p>
					</td>
					<td class="inPrice">
						<%=spans(num2cur(SELF_PRICE/arrList_orderEa),"#222222","13","400")%>&nbsp;<%=spans(""&Chg_currencyISO&"","#222222","12","400")%>
						<%If DK_MEMBER_STYPE = "0" Then%>
						<br /><%=spans(num2curINT(SELF_PV/arrList_orderEa),"#ff3300","11","400")%><%=spans(""&CS_PV&"","#ff3300","10","400")%>
						<%End If%>
					</td>
					<td class="tcenter">
						<input type="hidden" name="ea" class="input_text" style="width:25px;" value="<%=arrList_orderEa%>" /><%=arrList_orderEa%>
					</td>
					<td class="inPrice">
						<%=spans(num2cur(SELF_PRICE),"#222222","13","400")%>&nbsp;<%=spans(""&Chg_currencyISO&"","#222222","12","400")%>
						<%If DK_MEMBER_STYPE = "0" Then%>
						<br /><%=spans(num2curINT(SELF_PV),"#ff3300","11","400")%><%=spans(""&CS_PV&"","#ff3300","10","400")%>
						<%End If%>
					</td>
				</tr>
			<%
	...
	<div class="cleft width100 TotalArea">
		<div class="AreaZoneWrap">
			<div class="width100" style="margin: 10px 0px;">
				<table <%=tableatt%> class="width100">
					...
					<tr>
						<th class="tright"><span class="ti t"><%=LNG_SHOP_ORDER_DIRECT_TABLE_14%></span></th>
						<td class="bor_r">
							<div class="priceArea"><span class="price PriceArea" id="PriceAreaID"><%=num2cur(TOTAL_GOODS_PRICE)%></span><span class="won"><%=Chg_CurrencyISO%></span></div>
						</td>
						<th><span class=""><%=LNG_SHOP_ORDER_DIRECT_TABLE_15%></span></th>
						<td class="bor_r">
							<div class="priceArea"><span class="price DeliveryArea" id="DeliveryAreaID"><%=num2cur(TOTAL_DELIVERYFEE)%></span><span class="won"><%=Chg_CurrencyISO%></span></div>
						</td>
						<th><span class=""><%=LNG_SHOP_ORDER_DIRECT_TABLE_16%></span></th>
						<td>
							<div class="priceArea"><span class="price LastArea" id="LastAreaID"><%=num2cur(TOTAL_SUM_PRICE)%></span><span class="won"><%=Chg_CurrencyISO%></span></div>
						</td>
						<td></td>
					</tr>
				</table>
			...
			<div class="disArea width100">
				<table <%=tableatt%> class="width100">
					...
					<tbody>
						<%If DK_MEMBER_STYPE = "0" Then%>
							<tr>
								<td class="tright" colspan="2">·<%=LNG_CS_ORDERS_TOTAL_PV%> </td>
								<td>
									<%If DK_MEMBER_STYPE = "0" Then%>
										<div class="pvArea"><span id="TOTAL_PV" class="totalPV"><%=num2curINT(TOTAL_PV)%></span><%=spans(""&CS_PV&"","#ff3300","10","400")%></div>
						...
						<%If isSHOP_POINTUSE = "T" Then%>
							...
							<tr>
								<td class="tright"><span style="margin-left:5px;">사용가능 <%=SHOP_POINT%> : <strong><%=num2CurINT(MILEAGE_TOTAL)%></strong> <%=SHOP_POINT%></span></td>
								<td class="tright bor_l2">·<%=SHOP_POINT%> 사용</td>
								<td class="">
									<div class="pointArea">
										<%If DK_MEMBER_TYPE = "COMPANY" Then%>
											<input type="text" name="useCmoney" class="input_gray01"  onKeyUp="toCurrency(this)" onBlur="toCurrency(this); checkUseCmoney(this);" value="0" <%=onlyKeys%> />
											<br />
											<!-- <input type="text" name="useCmoney2" class="input_gray01" onKeyUp="<%=toCurrency%>(this)" onBlur="<%=toCurrency%>(this); checkUseCmoney2(this);" value="0" <%=onlyKeys%> /><span style="margin-left:5px;">(<%=SHOP_POINT2%> &nbsp;<strong><%=num2CurINT(MILEAGE2_TOTAL)%></strong> <%=Chg_CurrencyISO%></span>) -->
	...
	<div class="cleft ordersInfo width100">
		...
		<table <%=tableatt%> class="fright" style="margin-top: -30px;">
			<tr>
				<td style="border:none; background-color: #eceff1;"><label><input type="radio" name="dInfoType" class="input_radio" value="" onClick="baseDeliveryInfo(this.form);" checked="checked" /> <%=LNG_TEXT_BASE_DELIVERYINFO%></label></td>
				<td style="border:none; background-color: #eceff1;"><label><input type="radio" name="dInfoType" class="input_radio" value="" onClick="openDeliveryInfo();" /> <%=LNG_TEXT_LATEST_DELIVERYINFO%></label>
					<a name="modal" href="/shop/pop_deliveryInfo.asp" id="deliveryInfoModal" title="<%=LNG_TEXT_LATEST_DELIVERYINFO%>" ></a>
				</td>
				<!-- <td style="border:none;"><label><input type="radio" name="dInfoType" class="input_radio" value="" onClick="emptyDeliveryInfo(this.form);" /> <%=LNG_TEXT_DIRECT_INPUT%></label></td> -->
			</tr>
		</table>
		<div>
			<table <%=tableatt%> class="width100">
				...
				<tr>
					...
					<td><input type="text" name="takeName" class="input_text1" value="<%=backword_br(strName)%>"  /></th>
				</tr>
				<!-- <tr>
					<th><%=LNG_SHOP_ORDER_DIRECT_TABLE_18%></th>
					<td>
						<input type="text" class="input_text1" name="takeTel" style="width:150px;" maxlength="15" <%=onLyKeys%> value="<%=strTel%>" />
					</td>
				</tr> -->
				<tr>
					<th><%=LNG_SHOP_ORDER_DIRECT_TABLE_19%> <%=startext%></th>
					<td>
						<input type="text" class="input_text1" name="takeMobile" style="width:150px;" maxlength="15" <%=onLyKeys%> value="<%=strMobile%>" />
					</td>
				</tr><tr>
					...
					<td class="zipTD">
						<%Select Case UCase(DK_MEMBER_NATIONCODE)%>
							<%Case "KR"%>
								<p><input type="text" class="input_text1" name="takeZip"   id="takeZipDaum" style="width:80px;background:#f5f5f5;" readonly="readonly" value="<%=strzip%>" />	<span id="takeZipBtn"><input type="button" class="txtBtn j_medium" onclick="execDaumPostcode('takes');" value="우편번호검색" /></span></p>
								<p><input type="text" class="input_text1" name="takeADDR1" id="takeADDR1Daum" style="width:95%;background:#f5f5f5;" readonly="readonly" value="<%=strADDR1%>" /></p>
								<p><input type="text" class="input_text1" name="takeADDR2" id="takeADDR2Daum" style="width:95%;" value="<%=strADDR2%>" /></p>
							<%Case "JPTTTTT"%>
								<p><input type="text" class="input_text1" name="takeZip"   id="takeZip" style="width:80px;background:#f5f5f5;" maxlength="7" readonly="readonly" value="<%=strzip%>" <%=onLyKeys3%> /> <input type="button" class="txtBtn j_medium" onclick="openzip_jp2EN('takeZip');" value="<%=LNG_TEXT_ZIPCODE%>" /> (ZipCode)</p>
								<p><input type="text" class="input_text1" name="takeADDR1" id="takeADDR1" style="width:95%;background:#f5f5f5;" readonly="readonly" value="<%=strADDR1%>" /></p>
								<p><input type="text" class="input_text1" name="takeADDR2" id="takeADDR2" style="width:95%;" value="<%=strADDR2%>" /></p>
							<%Case Else%>
								<p><input type="text" class="input_text1" name="takeZip"   id="takeZip"   style="width:80px;" maxlength="7" value="<%=strzip%>" <%=onLyKeys3%> /><span style="line-height:28px;"> (ZipCode)</span></p>
								<p><input type="text" class="input_text1" name="takeADDR1" id="takeADDR1" style="width:95%;" value="<%=strADDR1%>" /></p>
								<p><input type="text" class="input_text1" name="takeADDR2" id="takeADDR2" style="width:95%;" value="<%=strADDR2%>" /></p>
						<%End Select%>
					</td>
				</tr>
	...
	<div class="cleft width100 payment">
		...
		<div class="width100 selectPay">
			...
			%>
				...
				<label><input type="radio" name="paykind" value="CardAPI" onclick="chgPay(this.value)" class="input_radio" /> <%=LNG_SHOP_ORDER_DIRECT_PAY_01%>-수기</label>
				<%If 1=2 Then%>
				<label><input type="radio" name="paykind" value="mComplex" onclick="chgPay(this.value)" class="input_radio" /> 다카드결제</label>
				<%End If%>
				...
				<!-- <label><input type="radio" name="paykind" value="vBank" onclick="chgPay(this.value)" class="input_radio" /> <%=LNG_TEXT_VIRTUAL_ACCOUNT%></label> -->
			<%End If%>

			<%'▣포인트 단독결제%>
				<%If DK_MEMBER_TYPE = "COMPANY" And isSHOP_POINTUSE = "T" Then	%>
					<%If CDbl(MILEAGE_TOTAL) >= CDbl(TOTAL_SUM_PRICE) Then%>
						<label><input type="radio" name="paykind" value="point" onclick="chgPay(this.value)" class="input_radio"  /> 포인트 단독결제</label>
					   &nbsp;&nbsp;<span id="pointTXT" class="tweight red2 font14px" ></span>
					<%Else%>
						<!-- <label><input type="radio" name="paykind" value="point" onclick="chgPay(this.value)" class="input_radio" disabled="disabled" /> 포인트 단독결제 (포인트 부족)</label>
					   &nbsp;&nbsp;<span id="pointTXT" class="tweight red2 font14px" ></span> -->
					<%End If %>
				<%End If %>
			<%
		...
		<div class="cleft width100 payinfoWrap">
			<div class="fleft payTypeLeftArea">
						...
						<tr>
							<td style="padding:8px 0px;">
									...
									<table <%=tableatt%> class="width100">
										...
										<tr>
											...
											<td>
												...
												<input type="text" title="금액" name="CardPrice" class="input_text" style="width:75px" <%=onlykeys%> value="" />
											...
											<th>유효기간</th>
											<td>
												<select title="월" name="mCardMM" class="vmiddle input_text" style="width:60px;">
												...
												<select title="년" name="mCardYY" class="vmiddle input_text" style="width:80px;">
											...
											<th>비밀번호</th>
											<td>
												<input type="password" title="비밀번호 앞 2자리" name="mCardPass" class="input_text" style="width:40px" maxlength="2" value="" <%=onlyKeys%> /> (앞2자리)
											</td>
											<th>할부정보</th>
											<td>
												<select name="mQuotabase" class="vmiddle input_text">
								...
								<div class="paytypeAddArea">
									...
									<%If 1=2 Then%>
									<div class ="fright">
										<label class="addbtn design5"><input type="checkbox" name="BankChk" id="BankChk" value="T" class="input_chk2 addChk" > 무통장 추가</label>
										<label class="addbtn design5"><input type="checkbox" name="CashChk" id="CashChk" value="T" class="input_chk2 addChk" > 현금 추가</label>
									</div>
									<%End If%>
								</div>
								<%'복합결제-무통장%>
												...
												<%
													...
													If IsArray(arrList_B) Then
														For i = 0 To listLen_B
															...
															End If
												%>
													<li><label><input type="radio" name="mBankidx" class="input_radio" value="<%=arr_BankCode%>,<%=arr_BankName%>,<%=arr_BankPenName%>,<%=arr_BankAccountNumber%>" /> <%=arr_BankName%> | <strong><%=arr_BankAccountNumber%></strong> | <%=arr_BankPenName%></label>
												<%
														Next
													End If
												%>
									...
									<table <%=tableatt%> class="width100">
										...
										<tr>
											...
											<td>
												...
												<input type="text" title="금액" name="CashPrice"  id="CashPrice" class="input_text" style="width:100px" <%=onlykeys%> value="" />
						...
						<%Select Case DKPG_PGCOMPANY%>
							<%Case "SPEEDPAY","YESPAY","DAOU","NICEPAY","ONOFFKOREA","KSNET"%>
								<tr>
									<th>카드번호</th>
									<td>
										<input type="text" name="cardNo1" class="input_text tcenter" maxlength="4" onkeyup="focus_next_input(this);" style="width:60px;" placeholder="4자리" <%=onlyKeys%> value="" /> -
										<input type="password" name="cardNo2" class="input_text tcenter" maxlength="4" onkeyup="focus_next_input(this);" style="width:60px;" placeholder="4자리" <%=onlyKeys%> value="" /> -
										<input type="password" name="cardNo3" class="input_text tcenter" maxlength="4" onkeyup="focus_next_input(this);" style="width:60px;" placeholder="4자리" <%=onlyKeys%> value="" /> -
								...
								<tr class="lastTD">
									<th>할부정보</th>
									<td>
										<select name="quotabase" class="vmiddle input_text">
				...
				<div id="vBankInfo" class="width100" style="display:none;">
					<table <%=tableatt%> class="width100">
						...
						<tr>
							<th><%=LNG_SHOP_ORDER_DIRECT_PAY_14%></th>
							<td><input type="text" name="vBankDepName" class="input_text" /></td>
						</tr>
						<tr class="lastTD">
							<th>입금만료시간</th>
							<td>
				...
				<div id="Agreement">
					<div class="tweight ag">·<%=LNG_SHOP_ORDER_DIRECT_PAY_15%>
				...
				<div id="PG_POLICY_AREA" style="display:none;">
					<%If PG_POLICY_VIEW_01 = "T" Then%>
						<div id="PGAgreement">
							<div class="tweight ag">·<%=PG_POLICY_TITLE_01%>
								<div class="fright">
					...
					<%If PG_POLICY_VIEW_02 = "T" Then%>
						<div id="PGAgreement">
							<div class="tweight ag">·<%=PG_POLICY_TITLE_02%>
								<div class="fright">
					...
					<%If PG_POLICY_VIEW_03 = "T" Then%>
						<div id="PGAgreement">
							<div class="tweight ag">·<%=PG_POLICY_TITLE_03%>
								<div class="fright">
			...
			<div class="fleft" style="width:28%;">
				<div class="cleft width100 cart_btn">
					...
					<div class="btnArea">
						<span class="pL10"><input type="submit" class="txtBtnC medium red2 radius5" value="<%=LNG_SHOP_ORDER_DIRECT_PAY_22%>"/> </span>
					</div>
				</div>
			</div>
		</div>
...
<%
	...
	End Select
%>
<script type="text/javascript" src="/shop/order.bottom.js"></script>
