...
'	If RS_CPNO = "" Then Call alerts("주민번호가 입력되지 않았습니다(결제처리불가).\n\n본사에 문의해주세요.","back","")
	...
	If UNDER_MEMBER_ORDER_CHK > 1 Then Call ALERTS("회원번호당 2회까지만 구매할 수 있습니다","back","")	'▣더화이트
...
<%
	'▣더화이트 포인트
	If webproIP="T"  And DK_MEMBER_ID2 = 3337 Then
	'	MILEAGE_TOTAL = 821       'b포인트
	'	MILEAGE_TOTAL_A = 1500
	End IF

		MILEAGE_TOTAL_ORI = MILEAGE_TOTAL
		MILEAGE_TOTAL = Int(MILEAGE_TOTAL/2)	'반내림
		MILEAGE_TOTAL = 0						'B포인트 미사용으로 변경(2016-05-31)
%>
...
%>
	...
	<script type="text/javascript" src="orders_point.js"></script>
<%
End Select
%>
	...
	If webproIP="T" Then
		...
		If UCase(DKPG_PGCOMPANY) = "DAOU" Then
			If Right(DKFD_PGMOD,4) = "TEST" Then
				...
				Call ResRW("orders_point.js","자바연결2")
			Else
				Call ResRW(DKPG_PGIDS_SHOP_KEYIN,"DKPG_PGIDS_SHOP_KEYIN")
				'Call ResRW(DKPG_PGJAVA_BASE2,"자바연결2")
				Call ResRW("orders_point.js","자바연결2")
...
<div id="cart" class="orderList">
	<%=FORMDATA%>
		...
		<table <%=tableatt%> class="userCWidth orderTable">
			...
			<tfoot>
				...
				'	End If
					...
					If LAST_PRICE > (MILEAGE_TOTAL_A) Then Call ALERTS("포인트가 부족합니다.","back","")
				...
				<tr style="border-top:2px solid #cdcdcd;">
					<td colspan="2" class="tweight tright bg2 pR4">
						A <%=LNG_CS_ORDERS_USE_POINT%>(电子币)<span style="margin-left:5px;">(<%=LNG_CS_ORDERS_POINT_POSSESSION%> : <strong id="RemainArea"><%=num2Cur(MILEAGE_TOTAL_A)%></strong></span>)
					</td>
					<td class="tcenter">
					...
					<td rowspan="1" colspan="3" class="tweight tright bg2 pR4">결재할 잔여포인트</td>
					<td rowspan="1" class="inPrice tPrice2" ><span id="lastTXT"><%=num2cur(LAST_PRICE)%></span> POINT <span id="oriTXT3" style="display:none;"><%=num2cur(LAST_PRICE)%></span></td>
				</tr><tr>
					<td colspan="3" class="tweight tright bg2 pR4">
						<!-- B <%=LNG_CS_ORDERS_USE_POINT%>(现金币)<span style="margin-left:5px;">(<%=LNG_CS_ORDERS_POINT_POSSESSION%> : <strong id="RemainArea"><span class="mline"><%=MILEAGE_TOTAL_ORI%></span> → <%=num2Cur(MILEAGE_TOTAL)%></strong></span>) -->
					...
					<td rowspan="1" colspan="3" class="tweight tright bg2 pR4">포인트사용<!-- 합계(A+B) --></td>
		...
		<div id="DtoD_toggle">
		<p class="titles"><%=LNG_CS_ORDERS_DELIVERY_INFO%> - <span class="blue">매출받는 하선회원</span></p>
		<%
			arrParams = Array(_
				Db.makeParam("@mbid",adVarChar,adParamInput,20,DK_MEMBER_ID1), _
				Db.makeParam("@mbid2",adInteger,adParamInput,0,DK_MEMBER_ID2) _
		...
		<p class="titles"><%=LNG_TEXT_PASSWORD%> -USER (사용자 구매비밀번호)</p>
		<table <%=tableatt%> class="userCWidth">
			<col width="140" />
			<col width="620" />
			<tr>
				<th><%=LNG_TEXT_PASSWORD%> -USER <%=starText%></th>
				<td style="padding-left:5px;">
...
<input type="hidden" name="gopaymethod" value="point">
