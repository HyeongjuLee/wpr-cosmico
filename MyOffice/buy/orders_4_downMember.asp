...
'	If RS_CPNO = "" Then Call alerts("주민번호가 입력되지 않았습니다(결제처리불가).\n\n본사에 문의해주세요.","back","")
	...
	If inUidx = "" Then Call ALERTS(LNG_CS_ORDERS_ALERT01,"back","")

	DownMemID1 = Trim(pRequestTF("DownMemID1",True))
	DownMemID2 = Trim(pRequestTF("DownMemID2",True))
	'▣ 하선구매체크
	SQL1 = "SELECT ISNULL(COUNT([OrderNumber]),0) FROM [tbl_salesdetail]"
	SQL1 = SQL1 & " WHERE [mbid] = ? AND [mbid2] = ? "
	SQL1 = SQL1 & "		AND [ReturnTF] = 1 "		'정상판매
	arrParams = Array(_
		Db.makeParam("@mbid",adVarChar,adParamInput,20,DownMemID1), _
		Db.makeParam("@mbid2",adInteger,adParamInput,0,DownMemID2) _
	)
	...
	'하선인 이름
	SQL_U = "SELECT [M_Name] FROM [tbl_Memberinfo] WHERE [mbid] = ? AND [mbid2] = ? "
	arrParams = Array(_
		Db.makeParam("@mbid",adVarChar,adParamInput,20,DownMemID1), _
		Db.makeParam("@mbid2",adInteger,adParamInput,0,DownMemID2) _
	)
	Set DKRS = Db.execRs(SQL_U,DB_TEXT,arrParams,DB3)
	...
	arrParams = Array(_
		...
		Db.makeParam("@M_NAME",adVarWChar,adParamInput,100,DK_MEMBER_NAME),_
		Db.makeParam("@DownMemID1",adVarChar,adParamInput,20,DownMemID1),_
		Db.makeParam("@DownMemID2",adInteger,adParamInput,0,DownMemID2),_
		Db.makeParam("@DownMemName",adVarWChar,adParamInput,100,DownMemName),_
		Db.makeParam("@IDENTITY",adInteger,adParamOutput,0,0), _
		Db.makeParam("@OUTPUT_VALUE",adVarChar,adParamOutput,10,"ERROR") _
	)
	Call Db.exec("HJP_ORDER4DOWN_TEMP_INSERT",DB_PROC,arrParams,DB3)
...
<%
	...
	'▣더화이트 포인트
	MILEAGE_TOTAL_ORI = MILEAGE_TOTAL
	MILEAGE_TOTAL = Int(MILEAGE_TOTAL/2)	'반내림
	MILEAGE_TOTAL = 0						'B포인트 미사용으로 변경(2016-05-31) → A포인트만사용
...
<div id="cart" class="orderList">
	<%=FORMDATA%>
		...
		<table <%=tableatt%> class="userCWidth orderTable">
			...
			<%
				...
				For i = 0 To UBound(arrUidx)
					...
					'SQL = "SELECT * FROM [DK_CART4DOWN] WHERE [intIDX] = ?"
					arrParams = Array(_
					...
					Set DKRSS =  Db.execRS("HJP_CART4DOWN_ONE",DB_PROC,arrParams,DB3)
					...
					PRINT tabs(1)&"		<td class=""inPrice"">"&num2cur(RS_Price2)&" "&Chg_CurrencyISO&"</td>"
					...
					PRINT tabs(1)&"		<td class=""inPrice Price"">"&num2cur(SELF_PRICE)&" "&Chg_CurrencyISO&"</td>"
					PRINT tabs(1)&"	</tr>"
					...
					Call Db.exec("HJP_ORDER_TEMP_GOODS_4DOWN_INSERT",DB_PROC,arrParams2,DB3)
			...
			<tfoot>
				<tr>
					...
					<td class="inPrice tPrice" style="background-color:#fbfdd0;"><%=num2cur(TOTAL_PRICE)%>&nbsp;<%=Chg_CurrencyISO%></td>
				</tr>
				...
				'	End If
					...
					'If LAST_PRICE > (MILEAGE_TOTAL + MILEAGE_TOTAL_A) Then Call ALERTS("포인트가 부족합니다.","back","")
					If LAST_PRICE > (MILEAGE_TOTAL_A) Then Call ALERTS(LNG_JS_SHORT_OF_POINT,"back","")
				...
				<!-- <tr>
					...
					<td class="inPrice tPrice"><span id="priTXT"><%=num2cur(DELIVERY_PRICE)%></span> <%=Chg_CurrencyISO%> <span id="oriTXT2" style="display:none;"><%=num2cur(DELIVERY_PRICE)%></span></td>
				</tr> --><!-- <tr>
					<td colspan="6" class="tweight tright bg2 pR4"><%=LNG_CS_ORDERS_TOTAL_PRICE%></td>
					<td class="inPrice tPrice1" ><span id="totalTXT"><%=num2cur(LAST_PRICE)%></span> <%=Chg_CurrencyISO%> <span id="oriTXT3" style="display:none;"><%=num2cur(LAST_PRICE)%></span></td>
				</tr> -->


				<input type="hidden" name="cmoneyUseLimit" value="1"> <!-- 사용가능 최소 적립금 -->
				<input type="hidden" name="cmoneyUseMin" value="1"> <!-- 결제시 최소사용 적립금 -->
				<input type="hidden" name="cmoneyUseMax" value="<%=LAST_PRICE / 2%>"> <%'▣(카드)결제시 최대사용 적립금        'LAST_PRICE - 1000%>
				<input type="hidden" name="ownCmoneyA" value="<%=checkNumeric(MILEAGE_TOTAL_A)%>"> <%'회원 보유 마일리지 A%>
				<input type="hidden" name="ownCmoney" value="<%=checkNumeric(MILEAGE_TOTAL)%>"> <%'회원 보유 마일리지 B, 50%%>
				...
				<tr style="border-top:2px solid #cdcdcd;">
					<td colspan="2" class="tweight tright bg2 pR4">
						A <%=LNG_CS_ORDERS_USE_POINT%>(<%=LNG_TEXT_POINT_A%>)<span style="margin-left:5px;">(<%=LNG_CS_ORDERS_POINT_POSSESSION%> : <strong id="RemainArea"><%=num2Cur(MILEAGE_TOTAL_A)%></strong></span>)
					</td>
					<td class="tcenter">
						<input type="text" name="useCmoneyA" class="input_gray01" style="width:100px;background-color:#effafe;" onKeyUp="toCurrency(this)" onBlur="toCurrency(this); checkUseCmoney(this);" value="0" />
					</td>
					<td rowspan="1" colspan="3" class="tweight tright bg2 pR4"><%=LNG_CS_ORDERS_FINAL_TOTAL_PRICE%></td>
					<td rowspan="1" class="inPrice tPrice2" ><span id="lastTXT"><%=num2cur(LAST_PRICE)%></span>&nbsp;<%=Chg_CurrencyISO%><span id="oriTXT3" style="display:none;"><%=num2cur(LAST_PRICE)%></span></td>
				</tr>

				<input type="hidden" name="useCmoney" value="0" />
				<!-- <tr>
					<td colspan="3" class="tweight tright bg2 pR4">
						B <%=LNG_CS_ORDERS_USE_POINT%>(<%=LNG_TEXT_POINT_B%>)<span style="margin-left:5px;">(<%=LNG_CS_ORDERS_POINT_POSSESSION%> : <strong id="RemainArea"><span class="mline"><%=MILEAGE_TOTAL_ORI%></span> → <%=num2Cur(MILEAGE_TOTAL)%></strong></span>)
					</td>
					<td class="tcenter">
						<input type="text" name="useCmoney" class="input_gray01" style="width:100px;background-color:#effafe;" onKeyUp="toCurrency(this)" onBlur="toCurrency(this); checkUseCmoney(this);" value="0" />
					</td>
					<td rowspan="1" colspan="3" class="tweight tright bg2 pR4">포인트사용합계</td>
					<td rowspan="1" class="inPrice tPrice3" ><span id="lastTXT2"><%=num2cur(0)%></span> POINT <span id="oriTXT3" style="display:none;"><%=num2cur(LAST_PRICE)%></span></td>
				</tr> -->
		...
		<div id="DtoD_toggle">
		<p class="titles"><%=LNG_CS_ORDERS_DELIVERY_INFO%> - <span class="blue">收到销售额的 下线会员<!-- 매출받는 하선회원 --></span></p>
		<%
			arrParams = Array(_
				Db.makeParam("@mbid",adVarChar,adParamInput,20,DownMemID1), _
				Db.makeParam("@mbid2",adInteger,adParamInput,0,DownMemID2) _
		...
		<table <%=tableatt%> class="userCWidth meminfo">
			...
			<tbody>
				<tr>
					<th><%=LNG_TEXT_MEMID%> <%=starText%></th>
					<td class="blue tweight"><%=DownMemID1%>-<%=Fn_MBID2(DownMemID2)%></td>
				</tr>
				<tr>
					...
					<td>
						<%If UCase(DK_MEMBER_NATIONCODE)="KR" Then%>
							<input type="text" class="input_text" name="takeZip" style="width:80px;background-color:#f4f4f4;" readonly="readonly" value="<%=cutZip(RS_addcode1)%>" />
							<img src="<%=IMG_MYPAGE%>/zip_search.gif" width="68" height="20" alt="주소 찾기" class="vmiddle cp" onclick="openzip('<%=oriAdd%>');" />
							<input type="text" class="input_text" name="takeADDR1" style="width:390px;background-color:#f4f4f4;" readonly="readonly" value="<%=RS_Address1%>" />
						<%Else%>
							<input type="text" class="input_text" name="takeADDR1" style="width:400px;" value="<%=RS_Address1%>" />
							&nbsp;&nbsp;&nbsp;ZIP CODE
							<input type="text" class="input_text vmiddle" name="takeZip" style="width:60px;" maxlength="7"  <%=onLyKeys%> value="<%=RS_addcode1%>" />
						<%End If%>
					</td>
				</tr><tr>
					<th><%=LNG_TEXT_ADDRESS2%> <%=starText%></th>
					<td><input type="text" class="input_text" name="takeADDR2" style="width:550px;" value="<%=RS_Address2%>" /></td>
				</tr><tr>
		...
		<table <%=tableatt%> class="userCWidth">
			...
			<tr>
				...
				<td style="padding-left:5px;"><label><input type="radio" name="payKind" value="inBank" class="input_chk" onclick="payKindSelect(this.value)" checked="checked" />
				<%=LNG_TEXT_ORDER_BANK%></label></td>
				<td style="padding-left:5px;"><%=LNG_TEXT_ORDER_BANK%></td>
			</tr>
			<!-- <tr id="inBankInfo" style="display:none;"> -->
			<tr>
								...
								For i = 0 To listLenB
									...
									If DKCONF_SITE_ENC = "T" Then
										Set objEncrypter = Server.CreateObject ("Hyeongryeol.StringEncrypter")
											...
											objEncrypter.InitialVector = con_EncryptKeyIV
											If DKCONF_ISCSNEW = "T" Then										'▣CS신버전 암/복호화 추가
												If arrListB(3,i) <> "" Then arrListB(3,i) = objEncrypter.Decrypt(arrListB(3,i))
											End If
										Set objEncrypter = Nothing
									End If
			...
			<!-- <tr>
				...
				<td style="padding-left:5px;"><label><input type="radio" name="payKind" value="point" class="input_chk" onclick="payKindSelect(this.value)" checked="checked"/> <%=LNG_CS_ORDERS_TEXT56%></label></td>
				<td style="padding-left:5px;"></td>
			</tr> -->
			<%'End If%>
		</table>

		<p class="titles"><%=LNG_TEXT_PASSWORD_USER%></p>
		<table <%=tableatt%> class="userCWidth">
			<col width="140" />
			<col width="620" />
			<tr>
				<th><%=LNG_TEXT_PASSWORD_USER%>&nbsp;<%=starText%></th>
				<td style="padding-left:5px;">
					<input type="password" name="UserPassWord" class="input_text imes" maxlength="20" size="24" /></td>
				</td>
			</tr>
		</table>
...
<input type="hidden" name="gopaymethod" value="inBank">
...
%>
	...
	<input type="hidden" name="isDownOrder" value="T" />	<!-- 하선구매 -->
...
<%
	...
	Call Db.exec("HJP_ORDER4DOWN_TOTAL_PRICE_UPDATE",DB_PROC,arrParams,DB3)
