...
<%
	...
	If Not checkRef(houUrl &"/myoffice/member/underJoin01.asp") Then Call ALERTS(LNG_ALERT_WRONG_ACCESS,"back","")
...
<!--
	function join_idcheck() {
		...
		*/
		...
		createRequest();
		var url = '/common/ajax_idcheck.asp?ids='+ids.value;
	...
	function vote_idcheck() {
		openPopup("pop_voter.asp", "vote_idcheck", 100, 100, "left=200, top=200");
	}
	function spon_idcheck() {
		openPopup("pop_Sponsor.asp", "spon_idcheck", 100, 100, "left=200, top=200");
	}
	...
	*/
		if (f.agreement.value != 'T')
		{
			...
			document.location.href = '/myoffice/member/underJoin01.asp';
			return false;
		}
		if (f.gather.value != 'T')
		{
		...
		if (f.strID.value == "")
		{
			...
			if (f.strID.value.search('dana')>-1 || f.strID.value.search('DANA')>-1) {
				alert('아이디에 dana는 포함할 수 없습니다.')
				return false;
			}
			*/
...
<div id="joinStep" class="step03">
	<!-- <div class="title_area">
		<p class="c_b_title tweight" style=""><%=LNG_JOINSTEP03_U_BTITLE%></p>
		...
		<div class="tableArea">
			...
			<table <%=tableatt%> class="width100">
				...
				<tr class="line">
					...
					<td>
						<input type="text" name="strID" class="input_text imes vmiddle" maxlength="20" placeholder=""/>
					...
					<td colspan="3"><input type="password" name="strPass" class="input_text imes" maxlength="20" size="24" placeholder="" /><span class="summary"><%=LNG_TEXT_PASSWORD_TYPE%></span></td>
				</tr><tr>
					<th><%=LNG_TEXT_PASSWORD_CONFIRM%>&nbsp;<%=starText%></th>
					<td colspan="3"><input type="password" name="strPass2" class="input_text imes" maxlength="20" size="24" placeholder="" /></td>
				</tr>
			</table>
		</div>
		...
		<div class="tableArea">
			...
			<table <%=tableatt%> class="width100">
					...
					<td>
						...
						<%
							...
							Else
								...
								Call ALERTS(LNG_JOINSTEP03_U_TEXT10,"BACK","")
							End If
							Call closeRS(DKRS)

							If BASE_BUSINESS_CODE = "" Then Call ALERTS(LNG_JOINSTEP03_U_TEXT10,"BACK","")
						%>
				...
				<tr>
					...
					<td>
						<input type="text" name="sponsor" class="input_text vmiddle readonly" maxlength="12" size="24" readonly="readonly" onclick="spon_idcheck()" placeholder="" />
						<input type="button" class="txtBtn j_medium" onclick="spon_idcheck();" value="검색" />
					</td>
				</tr>
				<tr>
					<th><%=LNG_TEXT_BANKNAME%> <br />/ <%=LNG_TEXT_BANKNUMBER%></th>
					<td>
						...
						<%
							...
							Call closeRS(DKRS)
						%>
						<%=DKRS_BankName%> <%If DKRS_bankaccnt <> "" Then %>/<%=DKRS_bankaccnt%><%End If%>
					</td>
				</tr><tr>
