	...
	function fnSubmitReg(){
		...
		if (cardCheck == "F"){
			...
			$('html, body').animate({scrollTop : $("#CARD_INFO").offset().top-130}, 100);
			doubleSubmit = false;
			return false;
		}
		...
		itemTable.each(function(){
			...
			var itemCount = td.eq(0).find("input[name=regItemCount]").val();
			var itemCode = td.eq(1).find("select[name=regItemCode] option:selected").val();
		...
		for (var i=0;i<itemCountArray.length;i++){
			if ((parseInt(itemCountArray[i]) < 1 || itemCodeArray[i] == "")) {
			...
			$.ajax({
				...
				,success:function(data) {
					...
					if (retData.result == "success") {
						...
						location.href='/MyOffice/autoship/order_list_CMS.asp';
					} else {
						doubleSubmit = false;
						alert(retData.message);
					}
