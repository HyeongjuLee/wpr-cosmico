...
<%
	...
	totalPrice              = pRequestTF_JSON("totalPrice", True)
	itemIdxArray            = pRequestTF_JSON("itemIdxArray", True)
	itemCountArray          = pRequestTF_JSON("itemCountArray", True)
	itemCodeArray           = pRequestTF_JSON("itemCodeArray", True)
	...
	If Not HJRS.BOF And Not HJRS.EOF Then
		...
		If CDbl(RS_A_ProcAmt) < 1  Then PRINT "{""result"":""error"",""message"":"""&LNG_AUTOSHIP_REG_AJAX_05&"""}" :  Response.End

		If RS_A_Card_Dongle = "" Then PRINT "{""result"":""error"",""message"":"""&LNG_AUTOSHIP_REG_AJAX_06&"""}" :  Response.End
		If RS_A_Start_Date = "" Or RS_A_Month_Date = "" Or RS_A_ProcDay = "" Then
			PRINT "{""result"":""error"",""message"":"""&LNG_AUTOSHIP_REG_AJAX_07&"""}" :  Response.End
		End If
	...
	Else
		PRINT "{""result"":""error"",""message"":"""&LNG_AUTOSHIP_REG_AJAX_08&"""}" :  Response.End
	End If
	Call CloseRS(HJRS)
	...
	'If Not (Right(A_ProcDay, 2) = "05" Or Right(A_ProcDay, 2) = "15" Or Right(A_ProcDay, 2) = "25") Then
	'    PRINT "{""result"":""error"",""message"":""오토십 시작일은 5, 15, 25일만 선택 가능합니다.""}"
	'    Response.end
	'End If
	...
	If itemIdxArray <> "" Then
		Dim itemIdxData : Set itemIdxData = JSON.parse(join(array(itemIdxArray)))
	End If

	If itemCountArray <> "" Then
		Dim itemCountData : Set itemCountData = JSON.parse(join(array(itemCountArray)))
	End If

	If itemCodeArray <> "" Then
		Dim itemCodeData : Set itemCodeData = JSON.parse(join(array(itemCodeArray)))
	End If
	...
	If cardInfoChk = "T" Then
		...
		If chkA_Card_DongleIDX <> "" Then
			...
			If CStr(chkA_Card_Dongle) <> CStr(A_Card_Dongle_CHK) Then
				PRINT "{""result"":""error"",""message"":"""&LNG_AUTOSHIP_REG_AJAX_02&"""}"
				Response.End
			End If
		Else
	...
	For i = 0 to (itemIdxData.length-1)
		...
		If itemIdx <> "" Then '순번이 있으면 이미 있던 정보
