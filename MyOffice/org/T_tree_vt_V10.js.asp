	...
	$(document).ready(function() {
		...
		*/
		...
		$("#search_period").ready(function(){
			...
			//alert(T_Display);
			if (T_Display == "block") {
				//$("#etcBtn").text("기타조건검색닫기");
				$("#etcBtn").text("<%=LNG_TEXT_DATE_SEARCH%>");
			} else {
				//$("#etcBtn").text("기타조건검색열기");
				$("#etcBtn").text("<%=LNG_TEXT_DATE_SEARCH%>");
			}
		});
			...
			,success: function(xhrData) {
				...
				if (jsonData.result == 'SUCCESS') {
					...
					for(var i=0;i<jsonData.FaData.length;i++){
						...
						//innerHtml += '<td><a href="javascript:sFrmSubmit(\''+jsonData.FaData[i].Fa_mbid1+'\',\''+jsonData.FaData[i].Fa_mbid2+'\');">선택1</a></td>'
					...
					innerHtml += '<tr class=\"modalCategory1\">';
					innerHtml += '<td colspan=\'3\' class=\'notData\'><%=LNG_TEXT_NO_FAVORITE_MEMBERS%></td>';
					innerHtml += '</tr>';
		...
		$.ajax({
			...
			,success: function(xhrData) {
				...
				if (jsonData.result == 'SUCCESS') {
					...
					innerHtml += '<td colspan=\'3\' class=\'notData\'><%=LNG_AJAX_TREE_SS_V2_TEXT01%></td>';
					innerHtml += '</tr>';
			...
			,error:function(xhrData) {
				//alert("데이터 저장 중 에러가 발생했습니다.");
				alert("<%=LNG_CS_CART_AJAX_ALERT05%>FF");
			}
		});
	}
