...
<%
	...
	Select Case MODE
		Case "ADD"
			SQL = "SELECT * FROM [DKT_TREE_FAVORITE_VOTE] WITH(NOLOCK) WHERE [MBID] = ? AND [MBID2] = ?"
			arrParams = Array(_
			...
			If Not DKRS.BOF And Not DKRS.EOF Then
				...
				Else
					...
					SQL2 = "UPDATE [DKT_TREE_FAVORITE_VOTE] SET [strFavorite] = ? WHERE [MBID] = ? AND [MBID2] = ?"
					arrParams2 = Array(_
			...
			Else
				SQL2 = "INSERT INTO [DKT_TREE_FAVORITE_VOTE] (MBID,MBID2,strFavorite) VALUES (?,?,?)"
				arrParams2 = Array(_
					Db.makeParam("@MBID1",adVarChar,adParamInput,20,DK_MEMBER_ID1), _
					Db.makeParam("@MBID2",adInteger,adParamInput,4,DK_MEMBER_ID2), _
		...
		Case "DEL","DEL_LIST"			'DEL_LIST 추가
			...
			If Not DKRS.BOF And Not DKRS.EOF Then
				...
				If FN_IN_ARRAY(mbid,S_DKRSD_strFavorite) = True Then
					...
					SQL3 = "SELECT * FROM [DKT_TREE_FAVORITE_VOTE] WITH(NOLOCK) WHERE [MBID] = ? AND [MBID2] = ?"
					arrParams3 = Array(_
						Db.makeParam("@MBID1",adVarChar,adParamInput,20,DK_MEMBER_ID1), _
						Db.makeParam("@MBID2",adInteger,adParamInput,4,DK_MEMBER_ID2) _
					)
					...
					If Not DKRS3.BOF And Not DKRS3.EOF Then
						...
						If DKRS3_strFavorite = "" Or DKRS3_strFavorite = "," Then
							...
							SQL4 = "DELETE FROM [DKT_TREE_FAVORITE_VOTE] WHERE [MBID] = ? AND [MBID2] = ?"
							arrParams4 = Array(_
								Db.makeParam("@MBID1",adVarChar,adParamInput,20,DK_MEMBER_ID1), _
								Db.makeParam("@MBID2",adInteger,adParamInput,4,DK_MEMBER_ID2) _
							)
