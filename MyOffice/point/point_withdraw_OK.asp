...
'▣체크변수
	...
	Select Case Left(OrderNum,4)
		...
		Case "PW2_"		'Bo
			Call alerts(LNG_ALERT_WRONG_ACCESS&"1","BACK","")
		...
		Case "PW3_"		'Za
			Call alerts(LNG_ALERT_WRONG_ACCESS&"2","BACK","")
	...
	GO_URL = "point_withdraw.asp"&GO_URL_QS
	'마일리지123 분기 / 치환  =============================================E
...
'	Call ResRW(GO_URL,"GO_URL")
'Response.End
'▣ 소수점 비교에러 추가 S =====================
	MILEAGE_TOTAL = CDbl(CStr(MILEAGE_TOTAL))
'▣ 소수점 비교에러 추가 E =====================

	If CDbl(transPointTOTAL) < CDbl(MINIMUM_POINT) Then Call ALERTS(LNG_CS_POINT_TRANSFER2CASH_JS05&"01","GO",GO_URL)
	If transPoint Mod WITHDRAW_UNIT <> 0 Then Call ALERTS(LNG_CS_POINT_TRANSFER2CASH_OK_ALERT06& "("&num2cur(WITHDRAW_UNIT)&")","GO",GO_URL)	'포인트는 출금액 단위로 입력
...
'▣검증
	...
	If Not AHJRS.BOF And Not AHJRS.EOF Then
		...
		AHJRS_WITHDRAW_UNIT		= AHJRS("WITHDRAW_UNIT")
		...
		If CDbl(num2Cur(WITHDRAW_UNIT)) <> CDbl(AHJRS_WITHDRAW_UNIT) Then Call ALERTS("Data modulation (WITHDRAW_UNIT)!","GO",GO_URL)


		'▣최소출금 확인
		If CDbl(transPoint) < CDbl(num2Cur(MINIMUM_POINT)) Then
			Call ALERTS(LNG_MINIMUM_WITHDRAW_POINT&" : ("&MINIMUM_POINT&") !","GO",GO_URL)
		End If
...
<%
	...
	arrParams = Array(_
		...
		Db.makeParam("@MinusKind",adVarchar,adParamInput,2,"82"), _
		Db.makeParam("@v_OrderNumber",adVarchar,adParamInput,30,""), _
	...
	If Err.Number <> 0 Then
		Call ALERTS(LNG_ALERT_UPDATE_ERROR,"GO",GO_URL)
	Else
		If MOB_PATH = "/m" Then
			chgPath = "/m"
		Else
			chgPath = "/myoffice"
		End If
		Call ALERTS(LNG_CS_POINT_TRANSFER2CASH_OK_ALERT05,"GO",chgPath&"/point/point_list.asp"&GO_URL_QS)
	End If
